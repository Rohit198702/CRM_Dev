...
public static class Utility
{
    ...
    public static string GetGraphAccessToken(string tenantId, string clientId, string clientSecret)
    {
        ...
        {
            throw new Exception("TenantId is missing from web.config.");
        }

        if (string.IsNullOrEmpty(clientId))
        {
            throw new Exception("ClientId is missing from web.config.");
        }

        if (string.IsNullOrEmpty(clientSecret))
        {
            throw new Exception("ClientSecret is missing from web.config.");
        }
        ...
        using (StreamReader reader = new StreamReader(response.GetResponseStream()))
        {
            ...
            GraphTokenResponse token = serializer.Deserialize<GraphTokenResponse>(json);

            if (token == null || string.IsNullOrEmpty(token.access_token))
            {
                throw new Exception("Failed to obtain Microsoft Graph access token.");
            }
    ...
    public static ArrayList GetCalendars(string accessToken, string email)
    {
        Dictionary<string, Dictionary<string, object>> uniqueCalendars = new Dictionary<string, Dictionary<string, object>>();
        JavaScriptSerializer serializer = new JavaScriptSerializer();

        try
        {
            // 1. Get direct calendars
            string url = "https://graph.microsoft.com/v1.0/users/" + email + "/calendars?$select=id,name";
            ...
            using (StreamReader reader = new StreamReader(response.GetResponseStream()))
            {
                ...
                Dictionary<string, object> data = serializer.Deserialize<Dictionary<string, object>>(result);

                if (data.ContainsKey("value"))
                {
                    ArrayList calendars = (ArrayList)data["value"];

                    foreach (Dictionary<string, object> calendar in calendars)
                    {
                        string id = calendar["id"].ToString();

                        if (!uniqueCalendars.ContainsKey(id))
                            uniqueCalendars.Add(id, calendar);
                    }
                }
            }

            // 2. Get calendar groups
            url = "https://graph.microsoft.com/v1.0/users/" + email + "/calendarGroups?$select=id,name";
            ...
            ArrayList calendarGroups = new ArrayList();
            ...
            using (StreamReader reader = new StreamReader(response.GetResponseStream()))
            {
                ...
                if (data.ContainsKey("value"))
                    calendarGroups = (ArrayList)data["value"];
            }

            // 3. Get calendars from each group
            foreach (Dictionary<string, object> group in calendarGroups)
            {
                string groupId = group["id"].ToString();

                url = "https://graph.microsoft.com/v1.0/users/" + email +
                      "/calendarGroups/" + groupId + "/calendars?$select=id,name";
            ...
            // 4. Convert dictionary to ArrayList
            ArrayList allCalendars = new ArrayList();

            foreach (Dictionary<string, object> calendar in uniqueCalendars.Values)
            {
    ...
    public static bool DeleteFolderCalendarEventBySubject(string accessToken, string email, string subject, string calendarFolderId)
    {
        try
        {
            ...
            FolderId folderId = new FolderId(calendarFolderId);

            SearchFilter filter = new SearchFilter.ContainsSubstring(ItemSchema.Subject, subject, ContainmentMode.Substring, ComparisonMode.IgnoreCase);
            ...
            // results.Items[0].Delete(DeleteMode.SoftDelete, SendCancellationsMode.SendToNone);
            service.DeleteItems(new ItemId[] { results.Items[0].Id }, DeleteMode.SoftDelete, SendCancellationsMode.SendToNone, null);
    ...
    public static bool CreateFolderCalendarEvent(string accessToken, string email, string calendarFolderId, string subject, DateTime start, DateTime end, string body, string attachmentNames)
    {
        try
        {
            ExchangeService service = new ExchangeService(ExchangeVersion.Exchange2013_SP1);
            ...
            appointment.Save(new FolderId(calendarFolderId), SendInvitationsMode.SendToNone);

            UploadFolderCalendarAttachments(service, appointment, attachmentNames);
    ...
    private static void UploadFolderCalendarAttachments(ExchangeService service, Appointment appointment, string attachmentNames)
    {
        ...
        string[] files = attachmentNames.Split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries);

        foreach (string fileName in files)
        {
            string filePath = Path.Combine(Utility.CCT_Calendar(), fileName);
            ...
            appointment.Attachments.AddFileAttachment(fileName, filePath);            
        }

        appointment.Update(ConflictResolutionMode.AutoResolve, SendInvitationsOrCancellationsMode.SendToNone);
    }

    public static bool DeleteCalendarEventBySubject(string accessToken, string email, string subject, string calendarId)
    {
        try
        {
            string filter = Uri.EscapeDataString("contains(subject,'" + subject.Replace("'", "''") + "')");

            string url;

            if (!string.IsNullOrEmpty(calendarId))
            {
                url = "https://graph.microsoft.com/v1.0/users/" + email + "/calendars/" + calendarId + "/events?$filter=" + filter;
            }
            else
            {
                url = "https://graph.microsoft.com/v1.0/users/" + email + "/events?$filter=" + filter;
            }
            ...
            using (StreamReader reader = new StreamReader(response.GetResponseStream()))
            {
                ...
                JavaScriptSerializer serializer = new JavaScriptSerializer();
                Dictionary<string, object> result = serializer.Deserialize<Dictionary<string, object>>(responseText);
                ...
                object[] events = ((System.Collections.ArrayList)result["value"]).ToArray();
                ...
                Dictionary<string, object> evt = (Dictionary<string, object>)events[0];
                string eventId = evt["id"].ToString();

                HttpWebRequest deleteRequest = (HttpWebRequest)WebRequest.Create(
                    "https://graph.microsoft.com/v1.0/users/" + email + "/events/" + eventId);

                deleteRequest.Method = "DELETE";
                deleteRequest.Headers["Authorization"] = "Bearer " + accessToken;

                using (HttpWebResponse deleteResponse = (HttpWebResponse)deleteRequest.GetResponse())
                {
        ...
        catch (WebException ex)
        {
            if (ex.Response != null)
            {
                using (StreamReader reader = new StreamReader(ex.Response.GetResponseStream()))
                {
                    ...
                    AddEditException(new Exception(error, ex));
                }
            }
            else
            {
    ...
    public static bool CreateCalendarEvent(string accessToken, string email, string eventBodyJson, string attachmentNames, string calendarId)
    {
        try
        {
            string url;
            ...
            if (!string.IsNullOrEmpty(calendarId))
            {
                url = "https://graph.microsoft.com/v1.0/users/" + email + "/calendars/" + calendarId + "/events";
            }
            else
            {
            ...
            byte[] bytes = Encoding.UTF8.GetBytes(eventBodyJson);
            ...
            using (HttpWebResponse response = (HttpWebResponse)request.GetResponse())
            {
                ...
                {
                    JavaScriptSerializer serializer = new JavaScriptSerializer();
                    Dictionary<string, object> data = serializer.Deserialize<Dictionary<string, object>>(responseJson);

                    string eventId = Convert.ToString(data["id"]);
                    UploadCalendarAttachments(accessToken, email, eventId, attachmentNames);
    ...
    private static void UploadCalendarAttachments(string accessToken, string email, string eventId, string attachmentNames)
    {
        if (string.IsNullOrEmpty(attachmentNames))
        {
            return;
        ...
        foreach (string fileName in files)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(
                "https://graph.microsoft.com/v1.0/users/" + email + "/events/" + eventId + "/attachments");
    ...
    public static DataSet GetWeeklyShippingActivity(int Operation)
    {
        DataSet ds = new DataSet();
        commonclass1 cls = new commonclass1();
        cls.Return_DS(ds, "EXEC Get_CCTDashboard " + Operation);
        return ds;
    }
    ...
    public static List<MailAddress> ConvertToMailList(DataTable dt)
    {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow row in dt.Rows)
                {
                    try
                    {
                        ...
                        string name = row["DisplayName"] != DBNull.Value ? row["DisplayName"].ToString() : string.Empty;

                        if (!string.IsNullOrEmpty(email))
                        {
                            list.Add(new MailAddress(email.Trim(), name));
                        }
                    }
                    catch (Exception ex)
                    {
    ...
    public static bool SalesActivityPMEmailSwitch()
    {
        bool returnValue = false;
        BOLAppSetting ObjBOL = new BOLAppSetting();
        BLLAppSetting ObjBLL = new BLLAppSetting();
    ...
    public static bool KPIChinaSwitch()
    {
        bool returnValue = false;
        BOLAppSetting ObjBOL = new BOLAppSetting();
        BLLAppSetting ObjBLL = new BLLAppSetting();
    ...
    public static bool ProductionReportSwitch()
    {
        bool returnValue = false;
        BOLAppSetting ObjBOL = new BOLAppSetting();
        BLLAppSetting ObjBLL = new BLLAppSetting();
    ...
    public static bool ForecastingSummReportSwitch()
    {
        bool returnValue = false;
        BOLAppSetting ObjBOL = new BOLAppSetting();
        BLLAppSetting ObjBLL = new BLLAppSetting();
    ...
    public static bool NestingSwitch()
    {
        bool returnValue = false;
        BOLAppSetting ObjBOL = new BOLAppSetting();
        BLLAppSetting ObjBLL = new BLLAppSetting();
    ...
    public static DataSet AutoFillPNumber_NewSP(String PNumber)
    {
        DataSet ds = new DataSet();
        BOLManageProjects ObjBOL = new BOLManageProjects();
    ...
    public static DateTime? ConvertDate(string date)
    {
        ...
        DateTime result;
        bool success = DateTime.TryParseExact(mm + "/" + dd + "/" + yy, "MM/dd/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out result);
        if (success)
        {
            return result;
        }
    ...
    {
        return "This Record was Updated by another User. Please refresh the Page to view latest changes.";
    }
    ...
    public static String Login(string UserName, String Password, string ProductLine, string Location)
    {
        String Message = "";
        BOLLoginInfo ObjBOL = Utility.GetLoginInfo(UserName.Replace("'", ""), Password.Replace("'", ""));
        if (ObjBOL.EmployeeID == 0)
        {
            Message = "Invalid User or Account is not Active !!";
            LoginStamp(Location, UserName, Password);
        }
        else
        {
            ...
            else
            {
                ...
                {
                    Message = "Your Account Status Is InActive. Please Consult Administration.";
                    LoginStamp(Location, UserName, Password);
                }
                else
                {
                    ...
                    if (ProductLine == "0")
                    {
                        if (ObjBOL.RoleName == "CC" || ObjBOL.RoleName == "QT")
                        {
                            HttpContext.Current.Response.Redirect("~/CCT/FrmCCTDashboard.aspx");
                        }
                        else if (ObjBOL.RoleName == "BD")
                        {
                            HttpContext.Current.Response.Redirect("~/SalesManagement/FrmActivityOperationalDashboard.aspx");
                        }
                        else
                        {
    ...
    public static string GetUserIP()
    {
        try
        {
            ...
            // If behind proxy, first IP is real client IP
            if (!string.IsNullOrEmpty(ip))
            {
                if (ip.Contains(","))
                    ip = ip.Split(',')[0].Trim();
    ...
    public static string GetUserAgent()
    {
        ...
        {
            ua = context.Request.UserAgent.ToLower();
        }
    ...
    public static HttpBrowserCapabilities GetBrowser()
    {
        HttpContext context = HttpContext.Current;
        if (context == null)
        {
    ...
    public static LocationInfo ResolveIP(string ip)
    {
        ...
        try
        {
            ...
            //Add changes as per Ruth in 23 Feb 2026
            ServicePointManager.SecurityProtocol = (SecurityProtocolType)3072;

            string url = "https://free.freeipapi.com/api/json/" + ip;
            ...
            string city = data.ContainsKey("cityName") ? data["cityName"] as string ?? "" : "";
            string state = data.ContainsKey("regionName") ? data["regionName"] as string ?? "" : "";
            string country = data.ContainsKey("countryName") ? data["countryName"] as string ?? "" : "";


            info.City = city;
            info.State = state;
            info.Country = country;
            info.FullAddress = city + ", " + state + ", " + country;
        }
    ...
    private static void LoginStamp(string Location, string username, string password)
    {
        BOLLoginDetails ObjBOL = new BOLLoginDetails();
        BLLLoginDetails ObjBLL = new BLLLoginDetails();
        ObjBOL.Operation = 2;
        if (IsAuthorized())
        {
            ObjBOL.UserId = GetCurrentUser();
        }
        else
        {
            ObjBOL.Username = username;
            ObjBOL.Password = password;
        }
        ObjBOL.IPAddress = GetUserIP();
        ObjBOL.BrowserInfo = GetUserAgent();
        LocationInfo info = ResolveIP(ObjBOL.IPAddress);
        ObjBOL.Location = Location;
        ObjBOL.CountryName = info.Country;
        ObjBOL.CityName = info.City;
        ObjBOL.RegionName = info.State;
        ObjBLL.SaveLogs(ObjBOL);
    }

    private static void LogoutStamp()
    {
        BOLLoginDetails ObjBOL = new BOLLoginDetails();
        BLLLoginDetails ObjBLL = new BLLLoginDetails();
        ObjBOL.Operation = 3;
    ...
    public static void ExportGrids(List<GridView> grids, List<string> sheetNames, ExportMode mode, string fileName)
    {
        ...
        using (ExcelPackage excel = new ExcelPackage())
        {
            ...
            if (mode == ExportMode.MultipleSheets)
            {
                for (int i = 0; i < grids.Count; i++)
                {
                    ...
                    ExportSingleGridToWorksheet(grids[i], ws, 1, null);
                }
            }
            else
            {
                ...
                if (!string.IsNullOrEmpty(fileName))
                {
                    ...
                    currentRow += 2; // spacing before first grid
                }
                ...
                for (int i = 0; i < grids.Count; i++)
                {
                    ...
                    currentRow = ExportSingleGridToWorksheet(grids[i], ws, currentRow, title);
                }
            }
    ...
    private static int ExportSingleGridToWorksheet(GridView gridView, ExcelWorksheet ws, int startRow, string title)
    {
        ...
        int col;
        int gridStartRow = row;
        bool hasDefinedColumns = gridView.Columns.Count > 0;
        ...
        for (int i = 0; i < gridView.HeaderRow.Cells.Count; i++)
        {
            ...
            if (hasDefinedColumns && i < gridView.Columns.Count)
            {
                if (gridView.Columns[i] is BoundField)
                {
                    BoundField bf = (BoundField)gridView.Columns[i];
                    header = bf.HeaderText;
                }
                else
                {
                    header = gridView.HeaderRow.Cells[i].Text;
                }
            }
            else
            {
            ...
            ws.Cells[row, col].Value = CleanText(header);
            ws.Cells[row, col].Style.Font.Bold = true;
            double width = 20;

            if (i < gridView.Columns.Count)
            {
                Unit colWidth = gridView.Columns[i].ItemStyle.Width;

                if (colWidth == Unit.Empty)
                    colWidth = gridView.Columns[i].HeaderStyle.Width;

                if (colWidth != Unit.Empty)
                    width = colWidth.Value / 7.0; // approximate px -> Excel width
            }
        ...
        {
            GridViewRow gr = gridView.Rows[r];
            col = 1;

            for (int c = 0; c < gr.Cells.Count; c++)
            {
                if (hasDefinedColumns)
                {
                    if (c >= gridView.Columns.Count || !gridView.Columns[c].Visible)
                    {
                        continue;
                    }
                }

                string text = ExtractCellText(gr.Cells[c]);
                ...
                if (text.StartsWith("$"))
                {
                    string val = text.Substring(1).Replace(",", "");
                                         ...
                                         System.Globalization.CultureInfo.InvariantCulture, out num))
                    {
                        ...
                        ws.Cells[row, col].Style.HorizontalAlignment =
                ...
                else if (decimal.TryParse(text.Replace(",", ""), out num))
                {
                    ws.Cells[row, col].Value = num;
                    ws.Cells[row, col].Style.HorizontalAlignment =
                        OfficeOpenXml.Style.ExcelHorizontalAlignment.Right;
                ...
                else
                {
                    if (text.StartsWith("`"))
                    {
                        ...
                        ws.Cells[row, col].Value = text.Replace("`", "");
                    }
                    else
                    {
                        ws.Cells[row, col].Value = text;
                ...
                SD.Color color = gr.Cells[c].BackColor;

                if (color != SD.Color.Empty)
                {
        ...
        int gridEndRow = row - 1;

        if (totalCols > 0 && gridEndRow >= gridStartRow)
        {
            var range = ws.Cells[gridStartRow, 1, gridEndRow, totalCols];
        ...
        return row + 2; // spacing before next grid
    }
    ...
    public static MemoryStream ExportToExcelStream(GridView gridView, string sheetName)
    {
        // Store original settings
        bool originalAllowSorting = gridView.AllowSorting;
        bool originalAllowPaging = gridView.AllowPaging;
        ...
        try
        {
            ...
            using (ExcelPackage excel = new ExcelPackage())
            {
                ...
                if (gridView.Columns.Count == 0)
                {
                    for (int i = 0; i < gridView.HeaderRow.Cells.Count; i++)
                    {
                        headerText = GetHeaderTextFromHtml(gridView.HeaderRow.Cells[i].Text);
                        workSheet.Cells[1, excelColumnIndex].Value = headerText.Replace("&nbsp;", "").Replace("&amp;", "&").Replace("&quot;", " ");
                        workSheet.Cells["A1:" + GetColumnLetter(excelColumnIndex) + "1"].Style.Font.Bold = true;
                        workSheet.Column(excelColumnIndex).Width = 20;
                        excelColumnIndex++;
    ...
    private static string GetHeaderTextFromHtml(string html)
    {
    ...
    public static DataTable ReturnGaylordProjects(Int32 op, String ProjectName)
    {
    ...
    public static DataTable ReturnGaylordJobId(Int32 op, String PONumber)
    {
        BOLGaylordProjects ObjBOL = new BOLGaylordProjects();
        BLLGaylordProjects ObjBLL = new BLLGaylordProjects();
        DataTable dt;
        ObjBOL.PONumber = PONumber;
        ObjBOL.Operation = op;
        dt = ObjBLL.Return_DataSet(ObjBOL).Tables[0];
        return dt;
    }
    ...
    public static void ExportToExcel_MultipleGrids(List<GridView> grids, string filename)
    {
        try
        {
    ...
    public static string EmailDisplayNameAerowerks()
    {
        return "Aerowerks";
    }
    ...
    public static string ReworkPath()
    {
        return BasePath() + "Rework_Docs/";
    }

    public static string CCT_Calendar()
    {
        return BasePath() + "CCT_Calendar/";
    }
