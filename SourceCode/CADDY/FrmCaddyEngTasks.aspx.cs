...
public partial class CADDY_FrmCaddyEngTasks : System.Web.UI.Page
{
    ...
    public void Bind_NatureofTask()
    {
        try
        {
            ...
            {
                Utility.BindDropDownList(ddlNatureOfTask, ds.Tables[0]);
    ...
    private void ResetTaskInfo()
    {
        try
        {
            ...
            txtRecFromCustomer.Text = string.Empty;
            txtFwdToCustomer.Text = string.Empty;
    ...
    private void SaveDetails()
    {
        try
        {
            ...
            {
                ObjBOL.ItemNo = txtItemNo.Text.Trim();
            }
            if(ddlModelType.SelectedIndex>0)
            {
            ...
            if (txtRecFromCustomer.Text.Trim() != "")
            {
                ObjBOL.RecFromCustomer = Utility.ConvertDateFormat(txtRecFromCustomer.Text);
            }

            if (txtFwdToCustomer.Text.Trim() != "")
            {
                ObjBOL.FwdToCustomer = Utility.ConvertDateFormat(txtFwdToCustomer.Text);
            }
    ...
    protected void gvCadEngTasks_RowEditing(object sender, GridViewEditEventArgs e)
    {
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                txtRecFromCustomer.Text = ds.Tables[0].Rows[0]["RecFromCustomer"].ToString();
                txtFwdToCustomer.Text = ds.Tables[0].Rows[0]["FwdToCustomer"].ToString();
    ...
    protected void gvCadEngTasks_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            ...
            string TimeStamp = Convert.ToString(gvCadEngTasks.DataKeys[e.RowIndex].Values[1]);
            ObjBOL.Operation = 9;
            ObjBOL.EngProjectID = ID;
            ObjBOL.JobNo =ddlPNumberHeaderList.SelectedValue;
            if(TimeStamp != "")
            {
    ...
    private DataTable ReportCombinedData()
    {
        DataTable dt = new DataTable();
        try
        {
            clscon.Return_DT(dt, "EXEC [dbo].[Caddy_CombinedHoodandConveyor] ");
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
    ...
    private void GetCombinedReport()
    {
        try
        {
            ...
            {
                DataTable dt = new DataTable();
                dt = ReportCombinedData();
                rprt.Load(Server.MapPath("~/CADDY/rptEngTaskReportDaily_Combined.rpt"));
                if (dt.Rows.Count > 0)
                {
                    TextObject txtheader = (TextObject)rprt.ReportDefinition.ReportObjects["txtHeader"];
                    txtheader.Text = "CADDY ENGINEERING (CONVEYOR + HOOD) DAILY TASK REPORT";
                    rprt.SetDataSource(dt);
                    rprt.ExportToHttpResponse(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, Response, false, txtheader.Text);
                    rprt.Close();
                    rprt.Dispose();
