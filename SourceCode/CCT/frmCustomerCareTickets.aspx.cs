...
public partial class CCT_frmCustomerCareTickets : System.Web.UI.Page
{
    ...
    private void QueryParameter()
    {
        try
        {
            ...
            {
                string ticketNo = Request.QueryString["tid"];
                string jobId = ticketNo.Split('-')[0];
                ddlJobNo.SelectedValue = jobId;
                ddlJobNo_SelectedIndexChanged();

                ddlTicketNo.ClearSelection();
                ddlTicketNo.Items.FindByText(ticketNo).Selected = true;
                ddlTicketNo_SelectedIndexChanged();
            }
        }
        catch (Exception ex)
        {
    ...
    private void Bind_Controls()
    {
        try
        {
            ...
            {
                Utility.BindDropDownList(ddlDepartment, ds.Tables[11]);
            }
        }
        catch (Exception ex)
        {
    ...
    protected void ddlJobNo_SelectedIndexChanged()
    {
        try
        {
            if (ddlJobNo.SelectedIndex > 0)
            {
                if (ValidationCheckJobID() == true)
                {
                    ...
                    BindTickets(Convert.ToInt32(defval));
                    BindSummary();
                    Bind_ConveyorModel(ObjBOL.TJobID.ToString());
                }
            }
            else
            {
    ...
    private void btnCancel_Click_Event()
    {
        ...
        {
            txtTicketno.Text = String.Empty;
            txtConveyorModel.Text = String.Empty;
            txtModel.Text = String.Empty;
    ...
    protected void ddlTicketNo_SelectedIndexChanged()
    {
        try
        {
            if (ddlTicketNo.SelectedIndex > 0)
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (ds.Tables[0].Rows[0]["JobID"].ToString() != null)
                    {
                        string JobID = ds.Tables[0].Rows[0]["JobID"].ToString();
                        Bind_ConveyorModel(JobID);
                    }
                    if (ds.Tables[0].Rows[0]["DepartmentID"].ToString() != "0")
                    {
    ...
    protected void gvSummary_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        try
        {
            ...
            string TimeStamp = Convert.ToString(gvSummary.DataKeys[e.RowIndex].Values[1]);   
            ObjBOL.Operation = 9;
            ObjBOL.TicketID = ID;
            if(hfTimeStamp.Value != "-1" && hfTimeStamp.Value.Trim() != "")
            {
            ...
            ObjBOL.TimeStampDetails = long.Parse(TimeStamp);
    ...
    private void Bind_ConveyorModel(string JobID)
    {
        try
        {
            DataSet ds = new DataSet();
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                txtConveyorModel.Text = modelname;
            }
            else
            {
                txtConveyorModel.Text = String.Empty;
