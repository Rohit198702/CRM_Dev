...
public partial class ContactManagement_FrmConsultants : System.Web.UI.Page
{
        ...
        try
        {
            if (ddlConsultant.SelectedIndex > 0)
            {
                    ...
                    Dictionary<string, Action<DataRow>> assignments = new Dictionary<string, Action<DataRow>>
                {
                    ...
                    { "Country", d =>
                        {
                            if(ddlCountry.Items.FindByValue(Convert.ToString(d["ConsultantCountryID"])) != null)
                            {
                                ddlCountry.SelectedValue = Convert.ToString(d["ConsultantCountryID"]);
                            }
                            else
                            {
                    ...
                    { "PrefVendor1", d =>
                        {
                            ...
                            {
                                if(ddlPrefVendor1.Items.Count > 0)
                                {
                    ...
                    { "PrefVendor2", d =>
                        {
                            ...
                            {
                                if(ddlPrefVendor2.Items.Count > 0)
                                {
                    ...
                    { "PrefVendor3", d =>
                        {
                            ...
                            {
                                if(ddlPrefVendor3.Items.Count > 0)
                                {
                    ...
                    { "NatureofConsultant", d =>
                        {
                            ...
                            {
                                if(ddlnatureConsul.Items.Count > 0)
                                {
        ...
        try
        {
            if (hfCusId.Value != "-1")
            {
                if (ValidationCheckMember() == true)
                {
                    ...
                    if (ddlMemberStatus.SelectedIndex > 0)
                    {
                        ObjBOLMember.Status = Convert.ToInt32(ddlMemberStatus.SelectedValue);
                    }
    ...
    private void Reset_Member()
    {
        try
        {
            ...
            if (ddlMemberStatus.Items.Count > 0)
            {
                ddlMemberStatus.SelectedValue = "1";
            }
        }
        catch (Exception ex)
        {
    ...
    protected void gvMember_RowEditing(object sender, System.Web.UI.WebControls.GridViewEditEventArgs e)
    {
        try
        {
            ...
            Bind_Members();
            Label lblMemberStatus = gvMember.Rows[e.NewEditIndex].FindControl("lblUpdateStatus") as Label;
            DropDownList ddlMemberStatus = gvMember.Rows[e.NewEditIndex].FindControl("ddlEditStatus") as DropDownList;
    ...
    protected void gvMember_RowUpdating(object sender, System.Web.UI.WebControls.GridViewUpdateEventArgs e)
    {
        try
        {
            ...
            DropDownList ddlMemberStatus = (row.FindControl("ddlEditStatus") as DropDownList);
            if (ddlMemberStatus.SelectedIndex == 0)
            {
                Utility.ShowMessage_Error(Page, "Please Select Contact Details Status. !");
                (row.FindControl("ddlEditStatus") as DropDownList).Focus();
                return;
            }
            else
            {
        ...
        try
        {
            if (ddlConsultant.SelectedIndex > 0)
            {
                ...
                else
                {
                    ...
                    txtheader.Text = "Related Projects Report ";
                    rprt.ExportToHttpResponse(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, Response, false, txtheader.Text);
                }

            }

            //Response.Redirect("~/Reports/frmSalesReport.aspx");
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
    ...
    protected void btnMafsiRedirect_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddlConsultant.SelectedIndex > 0)
            {
                send = "?Id=" + ddlConsultant.SelectedValue;
            }
            Response.Redirect("~/Mafsi/FrmMafsiDashboardConsultant.aspx" + send, false);
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
