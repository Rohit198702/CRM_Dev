...
public partial class ContactManagement_FrmDealers : System.Web.UI.Page
{
        ...
        try
        {
            if (ddlDealer.SelectedIndex > 0)
            {
                    ...
                    Dictionary<string, Action<DataRow>> assignments = new Dictionary<string, Action<DataRow>>
                {
                   ...
                   { "TierClassification", d =>
                        {
                            if(ddlTierClassification.Items.FindByValue(Convert.ToString(d["TierClassification"])) != null)
                            {
                                ddlTierClassification.SelectedValue = Convert.ToString(d["TierClassification"]);
                            }
                            else
                            {
                                if(ddlTierClassification.Items.Count > 0)
                                {
                                    ddlTierClassification.SelectedIndex = 0;
                                }
                            }
                        }
                    },
    ...
    private void Reset()
    {
        try
        {
            ...
            ddlTierClassification.SelectedIndex = 0;
            ddlTierClassificationHeader.SelectedIndex = 0;
            ...
            if (ddlContactStatus.Items.Count > 0)
            {
                ddlContactStatus.SelectedValue = "1";
            }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            if (ValidationCheck() == true)
            {
                ...
                if (ddlTierClassification.SelectedIndex > 0)
                {
                    ObjBOL.TierClassification = Convert.ToInt32(ddlTierClassification.SelectedValue);
                }
    ...
    private Boolean ValidationCheckMember()
    {
        try
        {
            ...
            if (ddlContactStatus.SelectedIndex == 0)
            {
                Utility.ShowMessage_Error(Page, "Please Select Contact Details Status !");
                ddlContactStatus.Focus();
                return false;
            }
        }
        catch (Exception ex)
        ...
        try
        {
            if (ddlDealer.SelectedIndex > 0)
            {
                if (ValidationCheckMember() == true)
                {
                    ...
                    if (ddlContactStatus.SelectedIndex > 0)
                    {
                        ObjBOLMember.Status = Convert.ToInt32(ddlContactStatus.SelectedValue);
                    }
    ...
    protected void gvMember_RowEditing(object sender, System.Web.UI.WebControls.GridViewEditEventArgs e)
    {
        try
        {
            ...
            Bind_Members();
            Label lblMemberStatus = gvMember.Rows[e.NewEditIndex].FindControl("lblMemberContactStatus") as Label;
            DropDownList ddlMemberStatus = gvMember.Rows[e.NewEditIndex].FindControl("ddlMemberContactStatus") as DropDownList;
            if (lblMemberStatus.Text != "")
            {
                ddlMemberStatus.SelectedValue = lblMemberStatus.Text;
            }
    ...
    protected void gvMember_RowUpdating(object sender, System.Web.UI.WebControls.GridViewUpdateEventArgs e)
    {
        try
        {
            ...
            ObjBOLMember.Timestamp = long.Parse(gvMember.DataKeys[e.RowIndex].Values[1].ToString());
            ...
            //Label MemberStatus=(row.FindControl("lblMemberContactStatus") as Label);
            DropDownList ddlStatus = (row.FindControl("ddlMemberContactStatus") as DropDownList);
            if (ddlStatus.SelectedIndex > 0)
            {
                ObjBOLMember.Status = Convert.ToInt32(ddlStatus.SelectedValue);
            }
        ...
        try
        {
            if (ddlHeadOffice.SelectedIndex > 0)
            {
                if (ValidationCheckHeadOffice() == true)
                {
                    string message = "";
                    ObjBOL.Operation = 3;
                    ObjBOL.Timestamp = long.Parse(HfTimestamp_HeadOffice.Value);
                    ...
                    if (ddlTierClassificationHeader.SelectedIndex > 0)
                    {
                        ObjBOL.TierClassification = Convert.ToInt32(ddlTierClassificationHeader.SelectedValue);
                    }
        ...
        try
        {
            if (ddlHeadOffice.SelectedIndex > 0)
            {
                    ...
                    Dictionary<string, Action<DataRow>> assignments = new Dictionary<string, Action<DataRow>>
                {
                    ...
                    { "Timestamp", d =>
                        {
                            HfTimestamp_HeadOffice.Value = Convert.ToString(d["Timestamp"]);
                        }
                    },
                    { "CountryState", d =>
                        {
                    ...
                    { "Dealerstatus", d =>
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    ddlHeadOfficeStatus.SelectedIndex = 0;
                                }
                            }
                        }
                    },
                    { "TierClassification", d =>
                        {
                            if(ddlTierClassificationHeader.Items.FindByValue(Convert.ToString(d["TierClassification"])) != null)
                            {
                                ddlTierClassificationHeader.SelectedValue = Convert.ToString(d["TierClassification"]);
                            }
                            else
                            {
                                if(ddlTierClassificationHeader.Items.Count > 0)
                                {
                                    ddlTierClassificationHeader.SelectedIndex = 0;
                                }
                            }
    ...
    protected void gvDealerHOMember_RowEditing(object sender, System.Web.UI.WebControls.GridViewEditEventArgs e)
    {
        try
        {
            ...
            Bind_DealerHOMembers();
            Label lblHOMemberStatus = gvDealerHOMember.Rows[e.NewEditIndex].FindControl("lblHOContactStatus") as Label;
            DropDownList ddlHOMemberStatus = gvDealerHOMember.Rows[e.NewEditIndex].FindControl("ddlHOMemberContactStatus") as DropDownList;
            if (lblHOMemberStatus.Text != "")
            {
                ddlHOMemberStatus.SelectedValue = lblHOMemberStatus.Text;
            }
            ModalPopupExtenderShowParts.Show();
        }
        catch (Exception ex)
    ...
    protected void gvDealerHOMember_RowUpdating(object sender, System.Web.UI.WebControls.GridViewUpdateEventArgs e)
    {
        try
        {
            ...
            ObjBOLMember.Timestamp = long.Parse(gvDealerHOMember.DataKeys[e.RowIndex].Values[1].ToString());
            ...
            DropDownList ddlHOMemberStatus = (row.FindControl("ddlHOMemberContactStatus") as DropDownList);
            if (ddlHOMemberStatus.SelectedIndex > 0)
            {
                ObjBOLMember.Status = Convert.ToInt32(ddlHOMemberStatus.SelectedValue);
            }
            if (txtTitle1.Text == "")
            {
                Utility.ShowMessage_Error(Page, "Please Enter Title. !");
    ...
    private Boolean ValidationCheckHOMember()
    {
        try
        {
            ...
            if (ddlHOContactStatus.SelectedIndex == 0)
            {
                Utility.ShowMessage_Error(Page, "Please Select Contact Details Status. !");
                ddlHOContactStatus.Focus();
                ModalPopupExtenderShowParts.Show();
                return false;
            }
        }
    ...
    private void Reset_DealerHOMember()
    {
        try
        {
            ...
            if (ddlHOContactStatus.SelectedIndex > 0)
            {
                ddlHOContactStatus.SelectedValue = "1";
            }
        }
        catch (Exception ex)
        {
        ...
        try
        {
            if (ddlHeadOffice.SelectedIndex > 0)
            {
                if (ValidationCheckHOMember() == true)
                {
                    ...
                    if (ddlHOContactStatus.SelectedIndex > 0)
                    {
                        ObjBOLMember.Status = Convert.ToInt32(ddlHOContactStatus.SelectedValue);
                    }
                    msg = ObjBLLMember.SaveDealerMember(ObjBOLMember);
                    if (msg.Trim() != "")
                    {
    ...
    protected void btnMafsiRedirect_Click(object sender, EventArgs e)
    {
        try
        {
            string send = "";
            if (ddlDealer.SelectedIndex > 0)
            {
                send = "?Id=" + ddlDealer.SelectedValue;
            }

            Response.Redirect("~/Mafsi/FrmMafsiDashboardDealer.aspx" + send, false);
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
