...
public partial class ContactManagement_FrmShipper : System.Web.UI.Page
{
    ...
    {
        ddlShipper_SelectedIndexChanged();
    }

    private void ddlShipper_SelectedIndexChanged()
    {
        try
        {
            if (ddlShipper.SelectedIndex > 0)
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (ddlCountry.Items.FindByValue(Convert.ToString(ds.Tables[0].Rows[0]["CountryId"])) != null)
                    {
                        ...
                        if (ddlComapnyState.Items.FindByValue(Convert.ToString(ds.Tables[0].Rows[0]["StateID"])) != null)
                        {
                            ddlComapnyState.SelectedValue = Convert.ToString(ds.Tables[0].Rows[0]["StateID"]);
                        }
                        else
                        {
                            if (ddlComapnyState.Items.Count > 0)
                            {
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            if (ValidationCheck() == true)
            {
                ...
                if (msg.Trim() == "ER02")
                {
                    Utility.ShowMessage_Error(this, "Shipper already exists !!");
                    return;
                }
    ...
    public void SaveCompanyMembers()
    {
        try
        {
            if (ValidationMemberCheck() == true)
            {
                ...
                ObjBOL.LastName = txtLName.Text;
                ObjBOL.Phone = txtPhone.Text;
                ObjBOL.CellNo = txtCellNo.Text;
                ObjBOL.Email = txtEmail.Text;
                ObjBOL.TimeStamp = long.Parse(hfTimestamp_Member.Value);
    ...
    protected void gvShipperMember_RowEditing(object sender, GridViewEditEventArgs e)
    {
        try
        {
            ...
            hfTimestamp_Member.Value = gvShipperMember.DataKeys[e.NewEditIndex].Values[1].ToString();
            ...
            Label lblCellNo = gvShipperMember.Rows[e.NewEditIndex].FindControl("lblGridCellNo") as Label;
    ...
    protected void ddlCountry_SelectedIndexChanged()
    {
        try
        {
            if (ddlCountry.SelectedIndex > 0)
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    Utility.BindDropDownList(ddlComapnyState, ds.Tables[0]);
                }
                else
                {
                    ddlComapnyState.Items.Clear();
