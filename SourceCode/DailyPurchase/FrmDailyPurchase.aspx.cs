...
public partial class DailyPurchase_FrmDailyPurchase : System.Web.UI.Page
{
    ...
    protected void btnSaveDetail_Click(object sender, EventArgs e)
    {
        try
        {
            if (ValidationCheckDetail())
            {
                ...
                if (hfDetailID.Value != "-1")
                {
                    ...
                    ObjBOL.TimeStamp = long.Parse(HfTimestamp_Parts.Value);
                }
                else
                {
                    ObjBOL.Operation = 6;
                ...
                returnStatus = ObjBLL.Return_String(ObjBOL).Trim();
                if (returnStatus != "")
                {
                    if (returnStatus == "ER01")
                    {
        ...
        {
            Utility.AddEditException(ex, returnStatus);
        }
    }
    ...
    private void CalculateTotalCost()
    {
        ...
        {
            Utility.AddEditException(ex, txtUnitPrice.Text + " / " + txtOrderQty.Text);
        }
    }
    ...
    //{
    //    ddlPart_SelectedIndexChanged();
    //}

    //private void ddlPart_SelectedIndexChanged()
    //{
    //    try
    //    {
    //        ObjBOL.Operation = 12;
    ...
    //        if (ddlDepartment.Items.FindByValue(returnValue.Trim()) != null)
    //        {
    //            ddlDepartment.SelectedValue = returnValue;
    //        }
    //        else
    //        {
    //            ddlDepartment.SelectedIndex = 0;
