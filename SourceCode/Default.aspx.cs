...
public partial class _Default : System.Web.UI.Page
{
    ...
    BOLNestingEmail ObjBOLNT = new BOLNestingEmail();
    BLLNestingEmail ObjBLLNT = new BLLNestingEmail();
    ...
    DateTime FromDate_ProductionReport = new DateTime();
    DateTime ToDate_ProductionReport = DateTime.Now.AddMonths(3);
    int currentYear = DateTime.Now.Year;
    private static Dictionary<string, DateTime> userLastCall = new Dictionary<string, DateTime>();
    string FromDate_ForeCastingReport = string.Empty;
    string ToDate_ForecastingReport = string.Empty;
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                if (Utility.IsAuthorized())
                {
                    ...
                    BindQuotesandOrders();
                    GetDasboardData();
                    BindForecastingSummaryReport();
                    NestingReleaseonHold();
    ...
    private void AutomateEmail()
    {
        ...
        {
            KPIChina_EmailEntry();
            ProductionReport_EmailEntry();
            FollowupEmailModuleEntry();
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
    ...
    #region Nesting Release On Hold Status
    private DataTable ReportData_NT_ReleaseOnHold()
    {
        DataTable dt = new DataTable();
        try
        {
            DataSet ds = new DataSet();
            ObjBOLNT.Operation = 1;
            ds = ObjBLLNT.Return_DataSet(ObjBOLNT);
            if (ds.Tables[0].Rows.Count > 0)
            {
                dt = ds.Tables[0];
            }
    ...
    private void UpdateEmailSentRHColumn(string JobNo)
    {
        try
        {
            string msg = String.Empty;
            ObjBOLNT.Operation = 2;
            ObjBOLNT.JobID = JobNo;
            msg = ObjBLLNT.Return_String(ObjBOLNT);
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
    ...
    public void NestingReleaseonHold()
    {
        try
        {
            DataTable dtExcelSummary = ReportData_NT_ReleaseOnHold();

            // Validate DataTable
            if (dtExcelSummary == null || dtExcelSummary.Rows.Count == 0)
                return;

            // Distinct PM Emails
            var distinctPMs = dtExcelSummary.AsEnumerable()
                .Where(x => x["EmailTo"] != DBNull.Value &&
                            !string.IsNullOrEmpty(x["EmailTo"].ToString()))
                .Select(x => x["EmailTo"].ToString().Trim())
                .Distinct()
                .ToList();

            if (distinctPMs.Count == 0)
                return;

            foreach (string pmEmail in distinctPMs)
            {
                try
                {
                    List<MailAddress> sendList_ToEmail = new List<MailAddress>();
                    List<MailAddress> sendList_CcEmail = new List<MailAddress>();

                    // Validate and Add TO emails
                    if (!string.IsNullOrEmpty(pmEmail))
                    {
                        foreach (string email in pmEmail.Split(','))
                        {
                            if (!string.IsNullOrEmpty(email))
                            {
                                try
                                {
                                    sendList_ToEmail.Add(new MailAddress(email.Trim()));
                                }
                                catch
                                {
                                    // Invalid email - skip
                    ...
                    // Skip if no valid TO emails
                    if (sendList_ToEmail.Count == 0)
                        continue;

                    // Filter rows for current PM
                    string filter = "EmailTo = '" + pmEmail.Replace("'", "''") + "'";
                    DataRow[] filteredRows = dtExcelSummary.Select(filter);
                    ...
                    foreach (DataRow row in filteredRows)
                    {
                        ...
                        string JobNo = row[0].ToString() ?? "";
                        string JobName = row[1].ToString() ?? "";
                        string ReleaseDate = row[2].ToString() ?? "";
                        string Warehouse = row[3].ToString() ?? "";
                        string sendTo = row[4].ToString() ?? "";
                        string ccList = row[5].ToString() ?? "";
                        string PMId = row[6].ToString() ?? "";
                        string PMName = row[7].ToString() ?? "";
                        ...
                        // TO Email Validation
                        sendList_ToEmail = new List<MailAddress>();

                        if (!string.IsNullOrEmpty(sendTo))
                        {
                            foreach (string email in sendTo.Split(new char[] { ',' },
                        ...
                        // CC Email Validation
                        sendList_CcEmail = new List<MailAddress>();

                        if (!string.IsNullOrEmpty(ccList))
                        {
                            foreach (string email in ccList.Split(new char[] { ',' },
                                     StringSplitOptions.RemoveEmptyEntries))
                            {
                                if (!string.IsNullOrEmpty(email))
                                {
                                    try
                                    {
                                        sendList_CcEmail.Add(new MailAddress(email.Trim()));
                                    }
                                    catch
                                    {
                                        // Invalid email - skip
                        ...
                        // Skip if no TO email exists
                        if (sendList_ToEmail.Count == 0)
                            continue;

                        UpdateEmailSentRHColumn(JobNo);

                        // Send Mail
                        SendNestingReleaseOnHold_Prepare(JobNo, JobName, ReleaseDate, Warehouse, PMName, sendList_ToEmail, sendList_CcEmail);
                    }
                }
                catch (Exception ex)
                {
    ...
    private void SendNestingReleaseOnHold_Prepare(string JobNo, string JobName, string ReleaseDate, string Warehouse, string PMName, List<MailAddress> sendList_ToEmail, List<MailAddress> sendList_CcEmail)
    {
        try
        {
            try
            {
                if (Utility.NestingSwitch())
                {
                    string Message = string.Empty;
                    string Subject = string.Empty;
                    Message += "<!doctype><html lang='en'><head><meta charset = 'utf-8'><meta name = 'viewport' content = 'width=device-width, initial-scale=1'> ";
                    Message += " <title> Project Release On-Hold </title></head><body><table cellpadding='0' cellspacing='0' style='border-collapse:collapse;width:100%;font-family:Calibri;font-size:1.15rem'> ";
                    ...
                    Message += " <tr><td colspan='2'><h2 style='margin:0;font-size:1.15rem'>" + PMName + ",</h2> ";
                    Message += " <p style = 'margin-top:5px'>Attached is the list of projects whose <strong> Nature of Task </strong> is marked as <strong>Release on Hold</strong>.<br/>";
                    ...
                    Message += " <h1 style ='font-size:1.65rem;margin:.3rem 0 0;color:#000;text-align:center'>Job Details</h1> ";
                    Message += " </td></tr><tr><td style='width:1%;white-space:nowrap'>Job No</td><td style='font-weight:600;width:99%'>" + JobNo + " </td></tr>";
                    Message += " <tr style='background:#efefef'><td style='width:1%;white-space:nowrap'>Project Name</td><td style='font-weight:600;width:99%'> " + JobName + "</td></tr>";
                    Message += " <tr><td style='width:1%;white-space:nowrap'> Release Date </td><td style='font-weight:600;width:99%'>" + ReleaseDate + "</td></tr>";
                    Message += " <tr style='background:#efefef'><td style='width:1%;white-space:nowrap'> Warehouse </td><td style='font-weight:600;width:99%'> " + Warehouse + "</td> ";
                    Message += " <tr><td colspan = '2'>Thank you. <br /><br /> ";
                    Message += Do_Not_Reply + " </tr></table></td></tr></table></body></html> ";
                    Subject = "Project Marked as Release on Hold";
                    Nesting_ReleaseOnHold_SendEmail(Message, Subject, sendList_ToEmail, sendList_CcEmail);
                    sendList_ToEmail.Clear();
                    sendList_CcEmail.Clear();
                }
            }
            catch (Exception ex)
            {
    ...
    private void Nesting_ReleaseOnHold_SendEmail(String Message, String Subject, List<MailAddress> sendToList, List<MailAddress> ccList)
    {
        try
        {
    ...
    #region Forecasting Summary Data Report
    private DataSet ReportData_EmailData()
    {
        DataSet ds = new DataSet();
        try
        {
            string query = string.Empty;
            query = "EXEC Get_AutoEmail_Forecast_Summary_Excel";
            clscon.Return_DS(ds, query);
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
    ...
    private DataTable ReportData_ExcelSummary(string FromDate, string ToDate)
    {
        DataTable dt = new DataTable();
        try
        {
            string query = string.Empty;
            query = "EXEC Get_Forecast_Summary_V2_Excel '" + FromDate + "', '" + ToDate + "', ";
            query += " NULL ";
            clscon.Return_DT(dt, query);
            if (dt.Columns.Contains("Min Qty"))
            {
                dt.Columns.Remove("Min Qty");
            }

            if (dt.Columns.Contains("Max Qty"))
            {
                dt.Columns.Remove("Max Qty");
            }
        }
        catch (Exception ex)
        {
    ...
    private void BindForecastingSummaryReport()
    {
        try
        {
            DataSet ds = new DataSet();
            DataTable dtExcelSummary = new DataTable();
            DataTable ToEmail = new DataTable();
            DataTable CcEmail = new DataTable();
            List<MailAddress> sendList_FSToEmail = new List<MailAddress>();
            List<MailAddress> ccList_FSCcEmail = new List<MailAddress>();
            ds = ReportData_EmailData();
            if (ds.Tables.Count == 0) return;
            if (ds.Tables[0].Rows.Count > 0)
            {
                string alreadySentEmail = ds.Tables[0].Rows[0]["AlreadySent"].ToString();
                if (alreadySentEmail != "1")
                {
                    FromDate_ForeCastingReport = (ds.Tables[1].Rows.Count > 0)
                    ? ds.Tables[1].Rows[0]["StartDate"].ToString()
                    : string.Empty;

                    ToDate_ForecastingReport =
                        (ds.Tables[2].Rows.Count > 0)
                    ? ds.Tables[2].Rows[0]["EndDate"].ToString()
                    : string.Empty;
                    if (FromDate_ForeCastingReport != null && ToDate_ForecastingReport != null)
                    {
                        dtExcelSummary = ReportData_ExcelSummary(FromDate_ForeCastingReport, ToDate_ForecastingReport);
                        if (dtExcelSummary.Rows.Count > 0)
                        {
                            ToEmail = ds.Tables[3];
                            CcEmail = ds.Tables[4];
                            gvSummary.DataSource = dtExcelSummary;
                            gvSummary.DataBind();
                            ...
                            additionalHeaderCell.Text = "<span id=\"additionalHeader\">Forecast Report from " + FromDate_ForeCastingReport + " to " + ToDate_ForecastingReport + " </span>";
                            additionalHeaderCell.ColumnSpan = ds.Tables[0].Columns.Count;
                            additionalHeaderRow.Cells.Add(additionalHeaderCell);
                            gvSummary.Controls[0].Controls.AddAt(0, additionalHeaderRow);

                            var sendList = (ToEmail != null && ToEmail.Rows.Count > 0)
                                ? Utility.ConvertToMailList(ToEmail) : new List<MailAddress>();

                            var ccList = (CcEmail != null && CcEmail.Rows.Count > 0)
                                ? Utility.ConvertToMailList(CcEmail)
                                : new List<MailAddress>();

                            sendList_FSToEmail = sendList;
                            ccList_FSCcEmail = ccList;
                            SendForecastingEmail_Prepare(sendList_FSToEmail, ccList_FSCcEmail);
                        }
                    }
                }
            }
    ...
    private void SendForecastingEmail_Prepare(List<MailAddress> sendList_ForeCastingSummReport, List<MailAddress> ccList_ForeCastingSummReport)
    {
        try
        {
            try
            {
                if (Utility.ForecastingSummReportSwitch())
                {
                    string Subject = "Forecasting Summary Report (Auto - Delivery)";
                    string Message = string.Empty;
                    Message += "<!doctype><html lang='en'> ";
                    Message += "<head><meta charset = 'utf-8'><meta name = 'viewport' content = 'width=device-width, initial-scale=1'> ";
                    Message += " <title> Forecasting Summary Report </title> ";
                    Message += " </head> ";
                    Message += " <body> ";
                    Message += " <p>Please find attached Forecasting Summary report from " + FromDate_ForeCastingReport + " to " + ToDate_ForecastingReport + " generated and automatically sent by our CRM system. </p>";
                    Message += " <p>This report will be sent on 1st Date of every Month to manage Parts Forecasting.</p> ";
                    Message += " <p>Thank you.</p> ";
                    Message += " <p colspan='2' style='color:Red'>" + Do_Not_Reply + "</body></html> ";
                    ForecastingSummaryReport_SendEmail(Message, Subject, sendList_ForeCastingSummReport, ccList_ForeCastingSummReport);
                    sendList_ForeCastingSummReport.Clear();
                    ccList_ForeCastingSummReport.Clear();
                }
            }
            catch (Exception ex)
            {
    ...
    private void ForecastingSummaryReport_SendEmail(String Message, String Subject, List<MailAddress> sendToList, List<MailAddress> ccList)
    {
        try
        {
            if (sendToList.Count > 0)
            {
                MailMessage message = new MailMessage(new MailAddress(Utility.Email(), "CRM"), sendToList[0]);
                string fileName = "Forecasting Summary Report " + FromDate_ForeCastingReport.Replace('/', '_') + " to " + ToDate_ForecastingReport.Replace('/', '_') + ".xlsx";
                string mailbody = Message;
                message.Subject = Subject;
                message.Body = mailbody;

                string sheetName = "Forecasing Summary" + " " + DateTime.Now.ToString("MM/dd/yyyy");
                Stream reportStream = Utility.ExportToExcelStream(gvSummary, sheetName);
                if (reportStream != null && reportStream.Length > 0)
                {
                    reportStream.Position = 0;
                    message.Attachments.Add(new Attachment(reportStream, fileName, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"));
                }
    ...
    private DateTime GetMonday_KPIChina()
    {
        DateTime fridayBeforePreviousMonday = DateTime.MinValue;
        ...
        try
        {
            ...
            DateTime previousMonday = thisMonday.AddDays(-7);

            // Friday before that Monday (3 days earlier)
            fridayBeforePreviousMonday = previousMonday;//.AddDays(-3);
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
        ...
        return fridayBeforePreviousMonday;
    }

    private DateTime GetFriday_KPIChina()
    {
        DateTime today = DateTime.Today;
    ...
    private DataTable ReportData_KPIChina()
    {
        DataTable dt = new DataTable();
        try
        {
            string query = "EXEC Get_KPIChina 1, '" + GetMonday_KPIChina().ToShortDateString() + "', '" + GetFriday_KPIChina().ToShortDateString() + "'";
            clscon.Return_DT(dt, query);
        }
        catch (Exception ex)
        {
    ...
    private DataTable ReportData_KPIChinaAccuracy()
    {
        DataTable dt = new DataTable();
        try
        {
            string query = "EXEC Get_KPIChina_DwgQuality 1, '" + GetMonday_KPIChina().ToShortDateString() + "', '" + GetFriday_KPIChina().ToShortDateString() + "'";
            clscon.Return_DT(dt, query);
        }
        catch (Exception ex)
        {
    ...
    private Stream KPIChinaReport()
    {
        Stream reportStream = null;
        try
        {
            DataTable dt = ReportData_KPIChina();
            if (dt.Rows.Count > 0)
            {
                rprt.Load(Server.MapPath("~/Reports/rptKPIChina.rpt"));
                TextObject txtheader = (TextObject)rprt.ReportDefinition.ReportObjects["txtHeader"];
                txtheader.Text = "KPI Agilent From " + GetMonday_KPIChina().ToString("MM/dd/yyyy") + " to " + GetFriday_KPIChina().ToString("MM/dd/yyyy");
            ...
            else
            {
                ...
                //TextObject txtheader = (TextObject)rprt.ReportDefinition.ReportObjects["txtHeader"];
                //txtheader.Text = "KPI Agilent From " + GetMonday_KPIChina().ToShortDateString() + " to " + GetFriday_KPIChina().ToShortDateString();
    ...
    private Stream KPIChinaDwhQualityReport()
    {
        Stream reportStream = null;
        try
        {
            DataTable dt = ReportData_KPIChinaAccuracy();
            rprt = new ReportDocument();
            if (dt.Rows.Count > 0)
            {
                rprt.Load(Server.MapPath("~/Reports/rptKPIChina_DwgQuality.rpt"));
                TextObject txtheader = (TextObject)rprt.ReportDefinition.ReportObjects["txtHeader"];
                txtheader.Text = "KPI Agilent Accuracy Level From " + GetMonday_KPIChina().ToString("MM/dd/yyyy") + " to " + GetFriday_KPIChina().ToString("MM/dd/yyyy");
            ...
            else
            {
                //rprt.Load(Server.MapPath("~/Reports/rptNoDataFound.rpt"));
                //TextObject txtheader = (TextObject)rprt.ReportDefinition.ReportObjects["txtHeader"];
                //txtheader.Text = "KPI Agilent Accuracy Level From " + GetMonday_KPIChina().ToShortDateString() + " to " + GetFriday_KPIChina().ToShortDateString();
                ////rprt.ExportToHttpResponse(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, Response, false, txtheader.Text);
                //reportStream = (Stream)rprt.ExportToStream(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat);
            }
        }
    ...
    private void KPIChina_EmailEntry()
    {
        try
        {
            if (Utility.KPIChinaSwitch())
            {
                List<MailAddress> sendList_KPIChina = new List<MailAddress>();
                List<MailAddress> ccList_KPIChina = new List<MailAddress>();
                DataSet ds = new DataSet();
                DataTable dt = new DataTable();

                clscon.Return_DS(ds, "EXEC Get_KPIChina_Email 1");
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    if (!string.IsNullOrEmpty(email))
                    {
                        sendList_KPIChina.Add(new MailAddress(email, name));
                    }
                }
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    if (!string.IsNullOrEmpty(email))
                    {
                        ccList_KPIChina.Add(new MailAddress(email, name));
                    }
                }

                KPIChina_PrepareEmailAndSend(sendList_KPIChina, ccList_KPIChina);
            }
            //else
            //{
            //    Utility.ShowMessage_Error(Page, "KPI China Email disabled !");
            //}
        }
        catch (Exception ex)
        {
    ...
    private void KPIChina_PrepareEmailAndSend(List<MailAddress> sendList_KPIChina, List<MailAddress> ccList_KPIChina)
    {
        try
        {
            if (Utility.KPIChinaSwitch())
            {
                //string Subject = "KPI Agilent Reports From " + GetMonday_KPIChina().ToShortDateString() + " to " + GetFriday_KPIChina().ToShortDateString();
                string Subject = "KPI 报告更新（自动发送） | KPI Report Update (Auto - Delivery)";

                string Message = string.Empty;
                Message += "<!doctype><html lang='en'> ";
                Message += "<head><meta charset = 'utf-8'><meta name = 'viewport' content = 'width=device-width, initial-scale=1'> ";
                Message += " <title> KPI Agilent Reports </title> ";
                Message += " </head> ";
                Message += " <body> ";
                Message += " <p style = 'margin-top:5px'>请查收附件，这是我们 CRM 系统自动生成的最新 KPI 报告。</p> ";
                Message += " <p>这些报告将每两周发送一次，旨在帮助我们双方持续跟踪项目绩效并识别改进机会。</p> ";
                Message += " <p>请团队查阅报告并据此采取必要的跟进行动，以持续提升工作表现。</p> ";
                Message += " <p>如有任何问题，欢迎随时联系。</p> ";
                Message += " <p>谢谢！</p> ";
                Message += " <p>Please find attached the latest KPI reports generated and automatically sent by our CRM system.</p> ";
                Message += " <p>These reports will be sent every two weeks to help both teams consistently track project performance and identify areas for improvement.</p> ";
                Message += " <p>We kindly ask your team to review the reports and take appropriate follow-up actions to continue improving performance.</p> ";
                Message += " <p>Should you have any questions, feel free to contact us.</p> ";
                Message += " <p>Thank you.</p> ";
                Message += " <p colspan='2' style='color:Red'>" + Do_Not_Reply + "</body></html> ";

                KPIChina_SendEmail(Message, Subject, sendList_KPIChina, ccList_KPIChina);
                sendList_KPIChina.Clear();
                ccList_KPIChina.Clear();
            }
            //else
            //{
    ...
    private void KPIChina_SendEmail(String Message, String Subject, List<MailAddress> sendToList, List<MailAddress> ccList)
    {
        try
        {
            if (sendToList.Count > 0)
            {
                KPIEmailSentLog();
                MailMessage message = new MailMessage(new MailAddress(Utility.Email(), "CRM"), sendToList[0]);
                string KPIChinaFileName = "KPI Agilent From " + GetMonday_KPIChina().ToString("MM/dd/yyyy").Replace('/', '_') + " to " + GetFriday_KPIChina().ToString("MM/dd/yyyy").Replace('/', '_') + ".pdf";
                string KPIChinaAccuracyFileName = "KPI Agilent Accuracy From " + GetMonday_KPIChina().ToString("MM/dd/yyyy").Replace('/', '_') + " to " + GetFriday_KPIChina().ToString("MM/dd/yyyy").Replace('/', '_') + ".pdf";
                string mailbody = Message;
                message.Subject = Subject;
                message.Body = mailbody;
                //Attachment KPIChinaFile = new Attachment(KPIChinaReport(), KPIChinaFileName, "application/pdf");
                //Attachment KPIChinaAccuracyFile = new Attachment(KPIChinaDwhQualityReport(), KPIChinaAccuracyFileName, "application/pdf");
                //message.Attachments.Add(KPIChinaFile);
                //message.Attachments.Add(KPIChinaAccuracyFile);

                Stream reportStream = KPIChinaReport();
                if (reportStream != null && reportStream.Length > 0)
                {
                    reportStream.Position = 0;
                    message.Attachments.Add(new Attachment(reportStream, KPIChinaFileName, "application/pdf"));
                }

                Stream accuracyStream = KPIChinaDwhQualityReport();
                if (accuracyStream != null && accuracyStream.Length > 0)
                {
                    accuracyStream.Position = 0;
                    message.Attachments.Add(new Attachment(accuracyStream, KPIChinaAccuracyFileName, "application/pdf"));
                }
    ...
    private void KPIEmailSentLog()
    {
        try
        {
            clscon.Return_string("EXEC Get_KPIChina_Email 2");
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
    ...
    private DataTable ReportData_ProductionReport()
    {
        DataTable dt = new DataTable();
        try
        {
            string query = "EXEC [dbo].[Get_SalesUsaCanProduction] '" + FromDate_ProductionReport.ToShortDateString() + "','" + ToDate_ProductionReport.ToShortDateString() + "', 5 ";
            clscon.Return_DT(dt, query);
        }
        catch (Exception ex)
        {
    ...
    private Stream ProductionReport()
    {
        Stream reportStream = null;
        try
        {
            DataTable dt = ReportData_ProductionReport();
            if (dt.Rows.Count > 0)
            {
                rprt.Load(Server.MapPath("~/Reports/rptProductionWeeklyShopWise.rpt"));
                TextObject txtheader = (TextObject)rprt.ReportDefinition.ReportObjects["txtHeader"];
                txtheader.Text = "Gaffney Production Report " + FromDate_ProductionReport.ToString("MM/dd/yyyy") + " to " + ToDate_ProductionReport.ToString("MM/dd/yyyy");
                rprt.SetDataSource(dt);
                //rprt.ExportToHttpResponse(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, Response, false, txtheader.Text);
                reportStream = (Stream)rprt.ExportToStream(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat);
            }
    ...
    private void ProductionReport_EmailEntry()
    {
        try
        {
            if (Utility.ProductionReportSwitch())
            {
                List<MailAddress> sendList_ProductionReport = new List<MailAddress>();
                List<MailAddress> ccList_ProductionReport = new List<MailAddress>();
                DataSet ds = new DataSet();
                DataTable dt = new DataTable();

                clscon.Return_DS(ds, "EXEC Get_ProductionReport_Email 1");
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    if (!string.IsNullOrEmpty(email))
                    {
                        sendList_ProductionReport.Add(new MailAddress(email, name));
                    }
                }
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    if (!string.IsNullOrEmpty(email))
                    {
                        ccList_ProductionReport.Add(new MailAddress(email, name));
                    }
                }

                FromDate_ProductionReport = Convert.ToDateTime(ds.Tables[2].Rows[0][0].ToString());
                ToDate_ProductionReport = Convert.ToDateTime(ds.Tables[2].Rows[0][1].ToString());
                ProductionReport_PrepareEmailAndSend(sendList_ProductionReport, ccList_ProductionReport);
            }
            //else
            //{
            //    Utility.ShowMessage_Error(Page, "KPI China Email disabled !");
    ...
    private void ProductionReport_PrepareEmailAndSend(List<MailAddress> sendList_ProductionReport, List<MailAddress> ccList_ProductionReport)
    {
        try
        {
            if (Utility.ProductionReportSwitch())
            {
                string Subject = "Gaffney Production Report (Auto - Delivery)";

                string Message = string.Empty;
                Message += "<!doctype><html lang='en'> ";
                Message += "<head><meta charset = 'utf-8'><meta name = 'viewport' content = 'width=device-width, initial-scale=1'> ";
                Message += " <title> Gaffney Production Report </title> ";
                Message += " </head> ";
                Message += " <body> ";
                Message += " <p>Please find attached Gaffney Production report from " + FromDate_ProductionReport.ToString("MM/dd/yyyy") + " to " + ToDate_ProductionReport.ToString("MM/dd/yyyy") + " generated and automatically sent by our CRM system. </p>";
                Message += " <p>This report will be sent every Monday to track projects sent to Gaffney Facility.</p> ";
                Message += " <p>Thank you.</p> ";
                Message += " <p colspan='2' style='color:Red'>" + Do_Not_Reply + "</body></html> ";

                ProductionReport_SendEmail(Message, Subject, sendList_ProductionReport, ccList_ProductionReport);
                sendList_ProductionReport.Clear();
                ccList_ProductionReport.Clear();
            }
            //else
            //{
            //    Utility.ShowMessage_Error(Page, "Email disabled !");
    ...
    private void ProductionReport_SendEmail(String Message, String Subject, List<MailAddress> sendToList, List<MailAddress> ccList)
    {
        try
        {
            if (sendToList.Count > 0)
            {
                ProductionReportEmailSentLog();
                MailMessage message = new MailMessage(new MailAddress(Utility.Email(), "CRM"), sendToList[0]);
                string fileName = "Gaffney Production Report " + FromDate_ProductionReport.ToString("MM/dd/yyyy").Replace('/', '_') + " to " + ToDate_ProductionReport.ToString("MM/dd/yyyy").Replace('/', '_') + ".pdf";
                string mailbody = Message;
                message.Subject = Subject;
                message.Body = mailbody;

                Stream reportStream = ProductionReport();
                if (reportStream != null && reportStream.Length > 0)
                {
                    reportStream.Position = 0;
                    message.Attachments.Add(new Attachment(reportStream, fileName, "application/pdf"));
                }
    ...
    private void ProductionReportEmailSentLog()
    {
        try
        {
            clscon.Return_string("EXEC Get_ProductionReport_Email 2");
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
    ...
    private void FollowupEmailModuleEntry()
    {
        try
        {
    ...
    private void SendFollowUpEmail_Prepare()
    {
        try
        {
            if (Utility.InventoryEmailSwitch())
            {
                ...
                Message += " Thanks, <br/ > <strong> Aerowerks </strong> <br /> ";
    ...
    private void Send_EmailFollowUp(String Message, String Subject, List<MailAddress> sendToList, List<MailAddress> ccList)
    {
        try
        {
            if (sendToList.Count > 0)
            {
                MailMessage message = new MailMessage(new MailAddress("aerowerksmohali@gmail.com", "Aerowerks"), sendToList[0]);
    ...
    [WebMethod]
    public static List<ProjectedVsActual> GetCumulativeProjectsSalesVsActualSales()
    {
        List<ProjectedVsActual> projectVsActual = new List<ProjectedVsActual>();
        ...
        try
        {
           ...
           .CopyToDataTable();
            foreach (DataRow row in dtFinal.Rows)
            {
                ...
                int effectiveMonth = currentDate.Day >= 25 ? currentDate.Month : currentDate.AddMonths(-1).Month;

                bool isFutureMonth = Convert.ToInt32(row["SrNo"]) > effectiveMonth;

                projectVsActual.Add(new ProjectedVsActual
                {
                    Month = row["Month"].ToString(),
                    ProjectedSales = isFutureMonth ? 0 : Convert.ToDecimal(row["CumulativeProjected"]),
                    ActualSales = isFutureMonth ? 0 : Convert.ToDecimal(row["CumulativeActual"])
                });
            }
        }
        catch (Exception ex)
    ...
    private void GetDasboardData()
    {
        try
        {
            if (Utility.IsAuthorized())
            {
                ...
                {
                    divDashboard.Visible = false;
                    Response.Redirect("~/Default_new.aspx");
                }
            }
    ...
    private string FirstBlock(DataTable dt)
    {
        try
        {
            if (dt.Rows.Count == 2)
            {
                ...
                decimal actualSales = (dr["ActualSales"] == DBNull.Value ? 0m : Convert.ToDecimal(dr["ActualSales"])) + (dr_1["ActualSales"] == DBNull.Value ? 0m : Convert.ToDecimal(dr_1["ActualSales"]));
                ...
                lblUSSales.InnerHtml = "US - <strong>$" + (dr_1["ActualSales"] == DBNull.Value ? "0.00" : Convert.ToDecimal(dr_1["ActualSales"]).ToString("N2")) + "</strong>";
                lblCADSales.InnerHtml = " | CAD - <strong>$" + (dr["ActualSales"] == DBNull.Value ? "0.00" : Convert.ToDecimal(dr["ActualSales"]).ToString("N2")) + "</strong>";
                ...
                decimal canadaActual = (dr["ActualSales"] == DBNull.Value ? 0m : Convert.ToDecimal(dr["ActualSales"]));
                ...
                decimal usActual = (dr_1["ActualSales"] == DBNull.Value ? 0m : Convert.ToDecimal(dr_1["ActualSales"]));
    ...
    private void FirstBlock_V1()
    {
        try
        {
            ...
            if (ds.Tables[1].Rows.Count > 0)
            {
                gvSalesCumulativeCurrentMonth.DataSource = ds.Tables[1];
                gvSalesCumulativeCurrentMonth.DataBind();
            }
        }
        catch (Exception ex)
        {
    ...
    int totalTarget = 0;
    int totalActual = 0;
    int totalTarget_Cumulative = 0;
    int totalActual_Cumulative = 0;
    protected void gvProjectedVsActual_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                totalTarget += target;
                totalActual += actual;
                string month = DataBinder.Eval(e.Row.DataItem, "Month").ToString();
                if (DateTime.Now.ToString("MMMM") == month)
                {
                    totalTarget_Cumulative = Convert.ToInt32(DataBinder.Eval(e.Row.DataItem, "CumulativeProjected"));
                    totalActual_Cumulative = Convert.ToInt32(DataBinder.Eval(e.Row.DataItem, "CumulativeActual"));
                }
            }
            else if (e.Row.RowType == DataControlRowType.Footer)
            {
                ...
                e.Row.Cells[4].Text = "$" + totalTarget_Cumulative.ToString("N0");
                e.Row.Cells[5].Text = "$" + totalActual_Cumulative.ToString("N0");
    ...
    public void Bind_GridMonthlyQuotesandOrders()
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                // List of month names that have passed
                var passedMonths = Enumerable.Range(1, DateTime.Now.Month - 1)
                                 .Select(m => new DateTime(1, m, 1).ToString("MMMM"))
                                 .ToList();

                // Filter the DataTable
                var filteredRows = dt.AsEnumerable()
                                     .Where(r => passedMonths.Contains(r.Field<string>("MonthName")))
                                     .ToList(); // Convert to list so we can add last row

                // Always include the last row
                var lastRow = dt.Rows[dt.Rows.Count - 1];
                if (!filteredRows.Contains(lastRow))
                {
                    filteredRows.Add(lastRow);
                }

                // Create new DataTable from filtered rows
                DataTable filteredDt = filteredRows.Any() ? filteredRows.CopyToDataTable() : dt.Clone();

                gvQuotesandOrders.DataSource = filteredDt;
    ...
    protected void gvSalesCumulativeCurrentMonth_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.Header)
        {
            e.Row.Cells.Clear();
            ...
            DateTime dt = DateTime.Now.Day >= 25 ? DateTime.Now : DateTime.Now.AddMonths(-1);

            cell.Text = "YTD Through " + dt.ToString("MMMM");
            cell.ColumnSpan = 2;
            cell.HorizontalAlign = HorizontalAlign.Center;
            cell.CssClass = "fw-bold text-center";

            e.Row.Cells.Add(cell);
        }

        if (e.Row.RowType == DataControlRowType.DataRow && e.Row.RowIndex == 2)
        {
            Label lblMetric = (Label)e.Row.FindControl("lblMetric");
            Label lblValue = (Label)e.Row.FindControl("lblValue");

            e.Row.Cells[0].ColumnSpan = 2;
            if (lblMetric.Text.ToLower().Trim() == "below projected")
            {
                e.Row.Cells[0].ForeColor = Color.Red;
            }
            else if (lblMetric.Text.ToLower().Trim() == "above projected")
            {
            ...
            lblMetric.Text = lblMetric.Text + " " + lblValue.Text;
    ...
    protected void btnGenerateInvoice_Click(object sender, EventArgs e)
    {
        try
        {
            Response.Redirect("~/Administration/FrmInvoice.aspx", false);
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
