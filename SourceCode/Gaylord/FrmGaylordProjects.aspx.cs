...
public partial class Gaylord_FrmGaylordProjects : System.Web.UI.Page
{
    ...
    private void BindControls()
    {
        try
        {
            ...
            {
                Utility.BindDropDownList(ddlPONumber, ds.Tables[4]);
            }

            if (ds.Tables[5].Rows.Count > 0)
            {
                Utility.BindDropDownList(ddlProjectName, ds.Tables[5]);
            }

            if (ds.Tables[6].Rows.Count > 0)
            {
                Utility.BindDropDownList(ddlLayoutBy, ds.Tables[6]);
            }
        }
        catch (Exception ex)
        {
    ...
    private bool ValidationCheck()
    {
        try
        {
            ...
            if (txtPOReceivedDate.Text.Trim() == "" && txtPONumber.Text.Trim() != "")
            {
    ...
    private void FetchInfo(DataSet ds)
    {
        try
        {
            ...
            Dictionary<string, Action<DataRow>> assignments = new Dictionary<string, Action<DataRow>>
            {
                ...
                { "ReleaseToNesting", d => chkReleaseToNesting.Checked = Convert.ToBoolean(d["ReleaseToNesting"]) },
                { "POType", d =>
                    {
    ...
    private void btnSave_Click()
    {
        try
        {
            if (ValidationCheck())
            {
                ...
                if (chkReleaseToNesting.Checked)
                {
                    ObjBOL.ReleaseToNesting = true;
                }
    ...
    private void AfterSaveOrUpdateSequence()
    {
        try
        {
            ...
            if (ds.Tables[1].Rows.Count > 0)
            {
                Utility.BindDropDownList(ddlPONumber, ds.Tables[1]);
            }

            if (ds.Tables[2].Rows.Count > 0)
            {
                Utility.BindDropDownList(ddlProjectName, ds.Tables[2]);
            }

            if (ddlProjectName.Items.FindByValue(ds.Tables[0].Rows[0]["id"].ToString()) != null)
            {
                ddlProjectName.SelectedValue = ds.Tables[0].Rows[0]["id"].ToString();
                ddlProjectName_SelectedIndexChanged();
            }
            else
            {
                ddlPONumber.SelectedValue = ds.Tables[0].Rows[0]["id"].ToString();
    ...
    private void btnCancel_Click()
    {
        try
        {
            ...
            ddlPOType.SelectedValue = "1";
            ddlPOType_SelectedIndexChanged();
            chkReleaseToNesting.Checked = false;
    ...
    private bool ValidationCheck_ProjectParts()
    {
        try
        {
            ...
            if (ddlProjectName.SelectedIndex == 0 && ddlPONumber.SelectedIndex == 0)
            {
                Utility.ShowMessage_Error(Page, "Please select PO !");
                ddlPONumber.Focus();
                return false;
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        try
        {
            if (ValidationCheck_ProjectParts())
            {
                ...
                ObjBOL.Timestamp = long.Parse(HfTimestamp_Parts.Value.ToString());
    ...
    protected void gvPartDetail_RowEditing(object sender, GridViewEditEventArgs e)
    {
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                HfTimestamp_Parts.Value = row["Timestamp"].ToString();
    ...
    protected void gvShipmentHistory_RowEditing(object sender, GridViewEditEventArgs e)
    {
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                HfTimestamp_Shipments.Value = row["Timestamp"].ToString();
    ...
    protected void btnAddShipmentDetail_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ValidationCheckShipment())
            {
                ...
                ObjBOL.Timestamp = long.Parse(HfTimestamp_Shipments.Value);
                ObjBOL.ProjectPartId = Int32.Parse(HfProjectPartId.Value);
    ...
    protected void ddlPONumber_SelectedIndexChanged()
    {
        try
        {
            if (ddlPONumber.SelectedIndex > 0)
            {
                ...
                if (ddlProjectName.Items.FindByValue(ddlPONumber.SelectedValue) != null)
                {
                    ddlProjectName.SelectedValue = ddlPONumber.SelectedValue;
                }
                else
                {
                ...
                int id = Int32.Parse(ddlPONumber.SelectedValue);
                GetData(id);
            }
            else
            {
    ...
    protected void ddlProjectName_SelectedIndexChanged()
    {
        try
        {
            if (ddlProjectName.SelectedIndex > 0)
            {
                btnPreviewReport.Enabled = true;
                if (ddlPONumber.Items.FindByValue(ddlProjectName.SelectedValue) != null)
                {
                    ddlPONumber.SelectedValue = ddlProjectName.SelectedValue;
                }
                else
                {
                ...
                int id = Int32.Parse(ddlProjectName.SelectedValue);
                GetData(id);
            }
            else
            {
    ...
    private DataTable ReportData()
    {
        ...
        try
        {
            cls.Return_DT(dt, "EXEC GL_GetProjects 1, '" + HfPOId.Value + "' ");
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
    ...
    protected void btnFilter_Click(object sender, EventArgs e)
    {
        try
        {
            Response.Redirect("~/Gaylord/FrmGaylordProjectsReport.aspx", false);
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
