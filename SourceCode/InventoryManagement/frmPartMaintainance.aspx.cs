...
public partial class INVManagement_frmPartMaintainance : System.Web.UI.Page
{
        ...
        try
        {
            if (Utility.IsAuthorized())
            {
                ...
                if (ds.Tables.Count > 0)
                {
                    ...
                    //Disabled for Purchasing Lookups only
                    else if(checkUserAccess == "2")
                    {
                        hfcontrolaccess.Value = checkUserAccess;
                        EnableDisableControlRecursively(dvAccessControls, true, "control-access");
                        EnableDisableControlRecursively(dvAccessControls, true, "control-access-enabled");
                        EnableDisableControlRecursively(ddlProductCodeLookUp, false, "control-access-enabled");
                        EnableDisableControlRecursively(ddlPartInfo, false, "control-access-enabled");
                    }
                    else
                    {
                        hfcontrolaccess.Value = checkUserAccess;
    ...
    private void Bind_Controls()
    {
        try
        {
            ...
            {
                Utility.BindDropDownList(ddlPartType, ds.Tables[7]);
            }
        }
        catch (Exception ex)
        {
    ...
    protected void ddlProductCodeLookUp_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            ITWReportSwitch(ddlProductCodeLookUp.SelectedValue);
    ...
    private void ITWReportSwitch(string value)
    {
        try
        {
            if (value == "2")
            {
                btnITWReport.Visible = true;
            }
            else
            {
                btnITWReport.Visible = false;
            }
        }
        catch (Exception ex)
        {
        ...
        try
        {
            if (ddlPartInfo.SelectedIndex > 0)
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (ddlPartType.Items.FindByValue(dr["PartTypeId"].ToString()) != null)
                    {
                        ddlPartType.SelectedValue = dr["PartTypeId"].ToString();
                    }
                    else
                    {
                    ...
                    ITWReportSwitch(ddlProductCode.SelectedValue);
    ...
    private bool SavePartDetails()
    {
        try
        {
            ...
            if (ValidationCheck() == true)
            {
                ...
                if (ddlPartType.SelectedIndex > 0)
                {
                    ObjBOL.PartTypeId = Convert.ToInt32(ddlPartType.SelectedValue);
                }
    ...
    protected void gvShopDwg_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            ...
            ObjBOL.Partid = ID;
            ObjBOL.PartLookupID = Convert.ToInt32(ddlPartInfo.SelectedValue);
            if (hfTimeStamp.Value != "-1")
            {
                ObjBOL.TimeStamp = long.Parse(hfTimeStamp.Value);
            }
