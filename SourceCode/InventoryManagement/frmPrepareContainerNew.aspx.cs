...
public partial class InventoryManagement_frmPrepareContainerNew : System.Web.UI.Page
{
    ...
    DataTable dt_WareHouseName = new DataTable();
    ...
    private void BindPartsDescOnDestWareHouse()
    {
        try
        {
            DataSet ds = new DataSet();
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                dt_WareHouseName = ds.Tables[0];
            }
            if (ds.Tables[1].Rows.Count > 0)
            {
                hfOldDestWareHouseID.Value = ds.Tables[1].Rows[0]["WareHouseID"].ToString();
            }
            else
            {
                hfOldDestWareHouseID.Value = "-1";
            }
        }
        catch (Exception ex)
        {
    ...
    protected void ddlDestWareHouse_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                string dbWareHouse = string.Empty;
                ...
                BindPartsDescOnDestWareHouse();
                if (dt_WareHouseName.Rows.Count > 0)
                {
                    dbWareHouse = dt_WareHouseName.Rows[0]["Name"].ToString();
                    string newWareHouse = ddlDestWareHouse.SelectedItem.Text;
                    if (dbWareHouse == newWareHouse)
                    {
                        Bind_GridChangeContainer();
                    }
                    else
    ...
    private void Bind_GridByVendor()
    {
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                DataTable source = HttpRuntime.Cache["dt"] as DataTable;
                if (source == null || source.Rows.Count == 0) return;

                var groupedData = source.AsEnumerable()
                    .GroupBy(r => r.Field<int>("Partid"))
                    .ToDictionary(g => g.Key, g => g.CopyToDataTable());
                Session["GroupedData"] = groupedData;
    ...
    private void Bind_Grid()
    {
        try
        {
            if (Utility.IsAuthorized())
            {
                ...
                if (hfOldDestWareHouseID.Value != "-1")
                {
                    ObjBOL.WarehouseID = Convert.ToInt32(hfOldDestWareHouseID.Value);
                }
                ds = ObjBLL.GetBindControl(ObjBOL);
                if (ds.Tables[0].Rows.Count > 0)
                {
    ...
    private Boolean ValidationCheck()
    {
        try
        {
            ...
            if (ddlDestWareHouse.SelectedIndex == 0 || ddlDestWareHouse.SelectedIndex == -1)
            {
            ...
            if (txtsentdate.Text == "")
            {
                Utility.ShowMessage_Error(Page, "Please Enter Sent Date (Load Date). !");
                txtsentdate.Focus();
                return false;
            }
    ...
    private void SaveContainerInfo()
    {
        try
        {
            if (Utility.IsAuthorized())
            {
                ...
                if (txtTentativeSentDate.Text.Trim() != "")
                {
                ...
                if (txtArrivalinAerowerks.Text.Trim() != "")
                {
                ...
                if (SaveGridData() == true)
                {
                    ...
                    containerProjects.Visible = true;
                    CheckStatus();
                    //Disabled for Agilent and Triflex for now and select the particular one
                    //Else Enabled 
                ...
                if (row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    foreach (GridViewRow Containerrow in gvContainer.Rows)
                    {
                        if (Containerrow.RowType == DataControlRowType.DataRow)
                        {
                            ...
                            if (ShippedQuantity > OrderQuantity && ItemRemarks.Text.Trim() == "")
                            {
                                Utility.ShowMessage_Error(Page, "Please Add Remarks !");
                                ItemRemarks.Focus();
                                return false;
                            }
                            if (ShippedQuantity == 0 && ddlStatus.SelectedValue == "2" && ItemRemarks.Text.Trim() == "")
                            {
                                Utility.ShowMessage_Error(Page, "Please Add Remarks !");
                                ItemRemarks.Focus();
                                return false;
                    ...
                    Dictionary<string, Action<DataRow>> assignments = new Dictionary<string, Action<DataRow>>
                    {
                        ...
                        { "WareHouseID",
                                d=>
                                {
                                    ...
                                    hfOldDestWareHouseID.Value=ddlDestWareHouse.SelectedValue;
                                }
    ...
    protected void btnPackingDetailsExcel_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                row.Cells[8].Style.Add("width", "100");
                row.Cells[8].Style.Add("Height", "110");
    ...
    private void reset()
    {
        try
        {
            ...
            btnPackingDetails.Enabled = false;
            btnPackingDetailsExcel.Enabled = false;
            if (ViewState["ContainerStatus"].ToString() == "True")
            {
                if (ddlDestWareHouse.Items.Count > 0)
                {
                    ddlDestWareHouse.SelectedIndex = 0; ;
    ...
    protected void gvMainRequisitionDetail_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType != DataControlRowType.DataRow)
            ...
            // Safety: ensure DataItem exists
            if (e.Row.DataItem == null)
                return;

            int partid = Convert.ToInt32(DataBinder.Eval(e.Row.DataItem, "Partid"));

            // Safety: check session object
            var groupedData = Session["GroupedData"] as Dictionary<int, DataTable>;
            if (groupedData == null || groupedData.Count == 0)
                return;

            GridView gvContainer = e.Row.FindControl("gvContainer") as GridView;
            if (gvContainer == null)
                return;
            DataTable dt;
            if (groupedData.TryGetValue(partid, out dt) && dt != null && dt.Rows.Count > 0)
            {
            ...
            {
                // Optional: clear grid if no data
