...
public partial class InventoryManagement_frmPurchaseOrderManual : System.Web.UI.Page
{
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ViewState["LoadChildData"] = null;
            ViewState["ChildGrdOnPageLoad"] = null;
            BindGridReqInfoBySource();
            HfTimeStamp.Value = "-1";
        }
        catch (Exception ex)
    ...
    private DataTable EmptyDTPurchaseOrder()
    {
        ...
        try
        {
            ...
            dt.Columns.Add(new DataColumn("ReqDeliDate", typeof(string)));
            ...
            dt.Columns.Add(new DataColumn("MUQ", typeof(int)));
                ...
                if (row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    foreach (GridViewRow PurchaseOrderrow in gvRequisitionInfo.Rows)
                    {
                        if (PurchaseOrderrow.RowType == DataControlRowType.DataRow)
                        {
                            ...
                            dr[15] = ItemRemarks.Text;
                            if (ddlStatus.SelectedIndex > 0)
                            {
                                dr[16] = Convert.ToInt32(ddlStatus.SelectedValue);
                            }
                            else
                            {
                                dr[16] = 1;
    ...
    private void ResetPOInformation()
    {
        try
        {
            ...
            {
                ddlShippingMethod.SelectedValue = "1";
            }
    ...
    private void Bind_GridBySource()
    {
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                // Create lookup grouped by PartId
                var lookup = dtChild.AsEnumerable().ToLookup(row => row.Field<int>("PartId"));
                HttpRuntime.Cache.Insert(
                    cacheKey,
                    lookup,                 // your DataTable
                    null,
    ...
    private void SaveData()
    {
        try
        {
            ...
            if (ddlPurchaseOrder.SelectedIndex > 0)
            {
                ...
                {
                    ObjBOL.TimeStamp = long.Parse(HfTimeStamp.Value.Split(',')[0]);
                }
            }
            else
            {
            ...
            if (ddlShippingMethod.Items.Count > 0)
            {
                ObjBOL.ShipmentMethod = Convert.ToInt32(ddlShippingMethod.SelectedValue);
            }            
            SaveGridData();
    ...
    private void AutoFillData()
    {
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                ddlShippingMethod.SelectedValue = ds.Tables[0].Rows[0]["ShipmentMethod"].ToString();
                ...
                if (ds.Tables[0].Rows[0]["TimeStamp"].ToString() != null)
                {
    ...
    private Boolean ValidationCheck()
    {
        try
        {
            ...
            {
                Utility.ShowMessage_Error(Page, "Please Select Shipping Method !");
    ...
    protected void gvMainPartDetail_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType != DataControlRowType.DataRow)
            ...
            int partId = (int)drv["PartId"];

            var lookup = HttpRuntime.Cache["ChildData"] as ILookup<int, DataRow>;

            if (lookup == null)
                return;

            GridView gvChild = (GridView)e.Row.FindControl("gvRequisitionInfo");
            if (gvChild == null)
                return;

            var rows = lookup[partId];

            if (rows.Any())
            {
                gvChild.DataSource = rows.CopyToDataTable(); // small subset only
                gvChild.DataBind();
            }
            else
            {
