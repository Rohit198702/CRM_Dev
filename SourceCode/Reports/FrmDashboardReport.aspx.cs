...
public partial class Reports_FrmDashboardReport : System.Web.UI.Page
{
    ...
    private void ExportToExcelInventory()
    {
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                gvInventoryReport.PagerSettings.Visible = false;
    ...
    private void BindInventoryGrid()
    {
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                ViewState["dirState"] = ds.Tables[0];
                if(hfBindPdf.Value == "-1")
                {
                    gvInventoryReport.AllowPaging = true;                    
                }
                else
                {
                    hfBindPdf.Value = "-1";
    ...
    private void ResetGrid()
    {
        try
        {
            ...
            gvInventoryReport.DataBind();
            //GridViewRow pagerRow = gvInventoryReport.BottomPagerRow;
            //DropDownList ddl = pagerRow.FindControl("ddlPageSize") as DropDownList;
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
    ...
    private void ddlProductLine_SelectedIndexChanged()
    {
        try
        {
            ...
            {
                ObjBOL.Status = Int32.Parse(ddlPartStatus.SelectedValue);
            }           
            ds = ObjBLL.Return_DataSet(ObjBOL);           
            if (ds.Tables[0].Rows.Count > 0)
            {
    ...
    private void ResetProductCode()
    {
        try
        {
            ...
            {
                ddlPartNo.Items.Clear();
                ddlPartNo.Items.Insert(0, new System.Web.UI.WebControls.ListItem("All", "0"));
            }
        }
        catch (Exception ex)
        {
    ...
    protected void ddlPartStatus_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ResetPartStatus();
    ...
    {
        get { return gvInventoryReport.PageIndex; }
    }


    protected void gvInventoryReport_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        try
        {
            int newPage = gvInventoryReport.PageIndex;

            string arg = e.NewPageIndex.ToString();
            if (arg == "Prev")
                newPage = gvInventoryReport.PageIndex - 1;
            else if (arg == "Next")
                newPage = gvInventoryReport.PageIndex + 1;
            else if (arg == "Last")
                newPage = gvInventoryReport.PageCount - 1;
            else // numeric button
                newPage = e.NewPageIndex;

            newPage = Math.Max(0, Math.Min(newPage, gvInventoryReport.PageCount - 1));

            gvInventoryReport.PageIndex = newPage;
            BindInventoryGrid(); // Rebind grid and pager
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
    ...
    protected void rptPager_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        try
        {
            if (e.CommandName == "Page")
            {
                ...
                gvInventoryReport.PageIndex = pageIndex;
                BindInventoryGrid(); // your method to bind the GridView
            }
        }
        catch (Exception ex)
        {
    ...
    protected void gvInventoryReport_DataBound(object sender, EventArgs e)
    {
        try
        {             
            GridViewRow pagerRow = gvInventoryReport.BottomPagerRow;
            ...
            int totalPages = gvInventoryReport.PageCount;
            int currentPage = gvInventoryReport.PageIndex;
            int pageGroupSize = 5;
    ...
    protected void ddlPageSize_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            gvInventoryReport.PageSize = pageSize;
            gvInventoryReport.PageIndex = 0;
            BindInventoryGrid();
        }
        catch (Exception ex)
    ...
    protected void gvInventoryReport_RowCreated(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.Pager)
            {
                ...
                if (ddl != null)
                {
                    ddl.SelectedValue = gvInventoryReport.PageSize.ToString();
                }
            }
        }
        catch (Exception ex)
