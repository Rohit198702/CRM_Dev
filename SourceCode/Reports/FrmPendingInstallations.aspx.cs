...
public partial class Reports_FrmPendingInstallations : System.Web.UI.Page
{
    ...
    private void BindControls()
    {
        try
        {
            ...
            clscon.Return_DS(ds, "EXEC Get_PendingInstallations 1");
            ...
            {
                Utility.BindDropDownListAll(ddlInstallationAssignedTo, ds.Tables[1]);
            }
        }
        catch (Exception ex)
        {
    ...
    private DataTable ReportData()
    {
        ...
        {
            var index = 0;
            string installationassignedto = String.Empty;
            ...
            if (ddlInstallationAssignedTo.Items.Count > 0)
            {
                
                if (ddlInstallationAssignedTo.SelectedIndex > 0)
                {
                    installationassignedto = ddlInstallationAssignedTo.SelectedValue;
                }
            }

            string query = "EXEC Get_PendingInstallations 2, '" + Utility.ConvertDate(txtDateFrom.Text) + "', '" + Utility.ConvertDate(txtDateTo.Text) + "', '" + technician + "', '" + rdbDateSettings.SelectedValue + "', '" + priority + "', '" + installationassignedto + "'";
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddlInstallationAssignedTo.SelectedIndex > 0)
            {
                ddlInstallationAssignedTo.SelectedIndex = 0;
            }
        }
        catch (Exception ex)
        {
