...
public partial class Reports_FrmSalesActivityReport_V2 : System.Web.UI.Page
{
    ...
    private DataTable ReportData(int type)
    {
        DataTable dt = new DataTable();
        try
        {
            ...
            string query = "EXEC Get_SalesActivityReport_V2 " + type + ", " + ddlDateType.SelectedValue + ", '" + txtDateFrom.Text + "', '" + txtDateTo.Text + "', " + ddlProjectManager.SelectedValue + ", " + selectedFilters;
        ...
        try
        {
            if (ValidationCheck())
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (ddlDateType.SelectedValue == "0")
                    {
                        dynamicDateLabel.Text = "Followup Date";
                    }
                    else
                    {
                        dynamicDateLabel.Text = "Followup Date";
                    ...
                    {
                        txtheader.Text = "Manufacturer Specification Report From " + txtDateFrom.Text + " to " + txtDateTo.Text;
                    }
                    else
                    {
                        txtheader.Text = "Manufacturer Specification Report ";
                    }
    ...
    protected void btnGenerateExcel_Click(object sender, EventArgs e)
    {
        try
        {
            if (ValidationCheck())
            {
                ...
                if (txtDateFrom.Text != "" && txtDateTo.Text != "")
                {
                    fileName = "Manufacturer Specification Report From " + txtDateFrom.Text + " to " + txtDateTo.Text;
                }
                else
                {
                    fileName = "Manufacturer Specification Report ";
                }
