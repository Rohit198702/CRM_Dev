...
public partial class Reports_frmContainerStatus : System.Web.UI.Page
{
    ...
    private void BindContainer()
    {
        try
        {
            ...
            qstr += "Select id as Containerid, CASE WHEN (Inv_Container.ContainerNo IS NULL OR  LTRIM(RTRIM(Inv_Container.ContainerNo)) = '') THEN Inv_Container.InvoiceNo ELSE   ";
        ...
        try
        {
            if (ValidationCheck() == true)
            {
                ...
                if (dt.Rows.Count > 0 || dt1.Rows.Count > 0)
                {
                    ...
                    rptGenerateReport.ReportSource = rprt;
                    rptGenerateReport.DataBind();
                    rprt.ExportToHttpResponse(CrystalDecisions.Shared.ExportFormatType.Excel, Response, false, fileName);
                }
                else
                {
                    rprt.Load(Server.MapPath("~/Reports/rptNoDataFound.rpt"));
