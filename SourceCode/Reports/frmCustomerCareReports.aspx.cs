...
public partial class Reports_frmCustomerCareReports : System.Web.UI.Page
{
    ...
    private System.Data.DataTable ReportDataSeventh()
    {
        System.Data.DataTable dt = new System.Data.DataTable();
        try
        {
            ...
            string strDateFrom = dtfrom.ToString("MM/dd/yyyy");
            string strDateTo = dtto.ToString("MM/dd/yyyy");
            clscon.Return_DT(dt, "EXEC [dbo].[aero_qryIndianProjects] '" + strDateFrom + "','" + strDateTo + "','" + 1 + "'");
        }
        catch (Exception ex)
        { 
    ...
    private void GenrateReport_Seventh()
    {
        try
        {
            System.Data.DataTable dt = ReportDataSeventh();
            DateTime dtfrom = Convert.ToDateTime(txtFromDate.Text);
            DateTime dtto = Convert.ToDateTime(txtToDate.Text);
            string strDateFrom = dtfrom.ToString("MM/dd/yyyy");
            string strDateTo = dtto.ToString("MM/dd/yyyy");
            rprt.Load(Server.MapPath("~/Reports/rptCustomerCareReports_IndianProjects.rpt"));
            if (dt.Rows.Count > 0)
            {
                TextObject txtheader = (TextObject)rprt.ReportDefinition.ReportObjects["txtHeader"];
                txtheader.Text = "Indian Projects Report From " + strDateFrom + " to " +strDateTo;
                rprt.SetDataSource(dt);
                rprt.ExportToHttpResponse(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, Response, false, txtheader.Text);
            }
            else
    ...
    private System.Data.DataTable Reportdata_SeventhExcel()
    {
        System.Data.DataTable dt = new System.Data.DataTable();
        try
        {
            ...
            string strDateFrom = dtfrom.ToString("MM/dd/yyyy");
            string strDateTo = dtto.ToString("MM/dd/yyyy");
            clscon.Return_DT(dt, "EXEC [dbo].[aero_qryIndianProjects] '" + strDateFrom + "','" + strDateTo + "','" + operation + "'");
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
    ...
    private void GenrateReport_SeventhExcel()
    {
        try
        {
            System.Data.DataTable dt = Reportdata_SeventhExcel();
            DateTime dtfrom = Convert.ToDateTime(txtFromDate.Text);
            DateTime dtto = Convert.ToDateTime(txtToDate.Text);
            string strDateFrom = dtfrom.ToString("MM/dd/yyyy");
            string strDateTo = dtto.ToString("MM/dd/yyyy");
            if (dt.Rows.Count > 0)
            {
                string filename = "Indian Projects";
                Utility.ExportToExcelDT(dt, filename);
            }
            else
            {
