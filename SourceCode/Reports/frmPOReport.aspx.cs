...
public partial class Reports_frmPOReport : System.Web.UI.Page
{
    ...
    private DataTable ReportData()
    {
        ...
        try
        {
            ...
            Qstr += " POId int, PartID int,  IsSubmitted int, ReqDetailID int, ReqDeliDate Varchar(50),MinUrgentQty INT  ) ";
            ...
            Qstr += " Inv_Parts.id,MIN(Inv_PurchaseOrder_Manual.IsSubmitted) AS IsSubmitted,MIN(Inv_RequisitionDetail.id) AS ReqDetailID,  ";
            Qstr += " MIN(Convert(Varchar(20),Inv_RequisitionDetail.RequestedArrivalDate,1)) as [ReqDeliDate],  ";
            Qstr += " MIN(Inv_RequisitionDetail.MinUrgentQty) AS [MinQty] FROM Inv_PurchaseOrderDetail_Manual ";
            ...
            Qstr += " UM,POId,PartID,IsSubmitted,ReqDeliDate,MinUrgentQty ";
            Qstr += " FROM @TempTable ";
    ...
    public void ExportNestedGridViewToExcel(GridView mainGridView)
    {
        try
        {
            using (ExcelPackage excel = new ExcelPackage())
            {
                ...
                workSheet.Cells[1, 12].Value = "Req. Dlv. Date";
                workSheet.Cells[1, 13].Value = "Requestor";
                workSheet.Cells[1, 14].Value = "Ship by";
                workSheet.Cells[1, 15].Value = "Order Qty";
                workSheet.Cells[1, 16].Value = "MUQ";
                workSheet.Cells[1, 17].Value = "UM";
                workSheet.Cells[1, 18].Value = "Ship Qty";
                workSheet.Cells[1, 19].Value = "Pending Qty";
                ...
                workSheet.Column(12).Width = 15; // Req. Dlv Date
                workSheet.Column(13).Width = 20; // Requestor
                workSheet.Column(14).Width = 15; // Ship by
                workSheet.Column(15).Width = 15; // Order Qty
                workSheet.Column(16).Width = 15; // MUQ
                workSheet.Column(17).Width = 10; // UM
                workSheet.Column(18).Width = 15; // Ship Qty
                workSheet.Column(19).Width = 15; // Pending Qty
                workSheet.Column(20).Width = 15; // Status
                int rowMain = 2;
                ...
                {                    
                    ...
                    string ReqDlrDate = ((Label)mainRow.FindControl("lblDlrDate")).Text;
                    ...
                    string muq = ((Label)mainRow.FindControl("lblMinQty")).Text;
                    ...
                    workSheet.Cells[rowMain, 12].Value = ReqDlrDate;
                    workSheet.Cells[rowMain, 13].Value = requestor;
                    workSheet.Cells[rowMain, 14].Value = shipBy;
                    workSheet.Cells[rowMain, 15].Value = orderQty;
                    workSheet.Cells[rowMain, 15].Style.HorizontalAlignment = ExcelHorizontalAlignment.Right;
                    workSheet.Cells[rowMain, 16].Value = muq;
                    workSheet.Cells[rowMain, 16].Style.HorizontalAlignment = ExcelHorizontalAlignment.Right;
                    workSheet.Cells[rowMain, 17].Value = um;
                    workSheet.Cells[rowMain, 18].Value = shipQty;
                    workSheet.Cells[rowMain, 18].Style.HorizontalAlignment = ExcelHorizontalAlignment.Right;
                    workSheet.Cells[rowMain, 19].Value = pendingQty;
                    workSheet.Cells[rowMain, 19].Style.HorizontalAlignment = ExcelHorizontalAlignment.Right;
                    workSheet.Cells[rowMain, 20].Value = status;
                    rowMain = rowMain + 1;
    ...
    private void BindPartNoOnSource(string sourceID)
    {
        try
        {
            ...
            if (ddlLookupPart.Items.Count > 1)
            {
                ddlLookupPart.SelectedIndex = 0;
            }
