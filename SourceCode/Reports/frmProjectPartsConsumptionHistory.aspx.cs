...
public partial class Reports_frmProjectPartsConsumptionHistory : System.Web.UI.Page
{
    ...
    private void Bind_DetailReport()
    {
        try
        {
            if (ValidationCheck() == true)
            {
                string headerText = "Project Parts Consumption History Detail Report ";
                if (txtProposalShipDateFrom.Text != "" && txtProposalShipDateTo.Text != "")
                {
                ...
                clscon.Return_DT(dt, "EXEC [dbo].[Get_ProjectPartsConsumption] 1,'" + strDateFrom + "','" + strDateTo + "'");
                rprt.Load(Server.MapPath("~/Reports/rptProjectPartsConsumptionHistory.rpt"));
                if (dt.Rows.Count > 0)
                {
    ...
    private void Bind_SummaryReport()
    {
        try
        {
            if (ValidationCheck() == true)
            {
                ...
                string strDateFrom = dtfrom.ToString("MM/dd/yyyy");
                string strDateTo = dtto.ToString("MM/dd/yyyy");
                clscon.Return_DT(dt, "EXEC [dbo].[Get_ProjectPartsConsumption] 2,'" + strDateFrom + "','" + strDateTo + "'");
                rprt.Load(Server.MapPath("~/Reports/rptProjectPartsConsumptionHistory_Summary.rpt"));
                if (dt.Rows.Count > 0)
                {
                    TextObject txtheader = (TextObject)rprt.ReportDefinition.ReportObjects["txtHeader"];
                    txtheader.Text = headerText;
    ...
    private void Bind_ReportExcel()
    {
        try
        {
            if (ValidationCheck() == true)
            {
                ...
                string strDateTo = dtto.ToString("MM/dd/yyyy");
                clscon.Return_DT(dt, "EXEC [dbo].[Get_ProjectPartsConsumption] 1,'" + strDateFrom + "','" + strDateTo + "'");
                Utility.ExportToExcelDT(dt, headerText);
            }
        }
        catch (Exception ex)
        {
    ...
    private void Bind_ReportSummaryExcel()
    {
        try
        {
            if (ValidationCheck() == true)
            {
                string headerText = "Project Parts Consumption History Summary Report ";
                if (txtProposalShipDateFrom.Text != "" && txtProposalShipDateTo.Text != "")
                {
                    headerText += "From " + txtProposalShipDateFrom.Text + " to " + txtProposalShipDateTo.Text;
                }
