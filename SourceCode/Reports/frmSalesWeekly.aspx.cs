...
public partial class Reports_frmSalesWeekly : System.Web.UI.Page
{
    ...
    {
        DataTable dt = new DataTable();
        clscon.Return_DT(dt, "SELECT ID,WarehouseName AS FacilityName FROM Inv_Warehouse ORDER BY WarehouseName ");
    ...
    public void CombinePdfs(string folderPath, List<string> fileNames, string fromdate, string todate)
    {
        ...
        try
        {
            ...
            using (FileStream fileStream = new FileStream(combinedfilePath, FileMode.Create))
            {
                ...
                Response.AddHeader("Content-Disposition", "attachment; filename=" + combinedfileName);
                Response.Buffer = false;

                using (FileStream fs = new FileStream(combinedfilePath, FileMode.Open, FileAccess.Read))
                {
                    byte[] buffer = new byte[81920]; // 80KB chunk
                    int bytesRead;
