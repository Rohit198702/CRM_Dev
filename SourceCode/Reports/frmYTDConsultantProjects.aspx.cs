...
public partial class Reports_frmYTDConsultantProjects : System.Web.UI.Page
{
    ...
    private void SetDates()
    {
        try
        {
            txtFromDate.Text = "01" + "/01/" + (DateTime.Now.Year - 3);
            txtToDate.Text = "12" + "/31/" + (DateTime.Now.Year - 1);
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < textObjects.Count; i++)
                    {
                        ...
                        if (i > 0)
                        {
                            ...
                            {
                                grandTotalObjects[i].Text = "$" + float.Parse(total.ToString()).ToString("N", new CultureInfo("en-US"));
                            }
                        }
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                Get_DealersReportPrimeSpec_Excel();
            }
            else
            {
                Get_DealersReportAlternate_Excel();
            }
        }
        catch (Exception ex)
        {
            ...
            if (ddlConsultant.SelectedValue == "0")
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < dt.Columns.Count; i++)
                    {
                        ...
                        {
                            totalRow[i] = dt.AsEnumerable()
                                            .Where(r => r[i] != DBNull.Value)
                                            .Sum(r => Convert.ToDecimal(r[i]));
                        }
                    }

                    dt.Rows.Add(totalRow);
                    Utility.ExportToExcelDT(dt, "Sales Report From " + ((Convert.ToDateTime(txtToDate.Text)).Year - 2) + " to " + (Convert.ToDateTime(txtToDate.Text)).Year);
                }
                else
                {
    ...
    private void Get_DealersReportPrimeSpec_Excel()
    {
        try
        {
            DateTime dtfrom = Convert.ToDateTime(txtFromDate.Text);
            ...
            if (ddlConsultant.SelectedValue == "0")
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    dt.Rows.Add(totalRow);
                    Utility.ExportToExcelDT(dt, "Prime Spec Report From " + ((Convert.ToDateTime(txtToDate.Text)).Year - 2) + " to " + (Convert.ToDateTime(txtToDate.Text)).Year);
                }
                else
                {
                    Utility.ShowMessage_Error(Page, "No Data found !");
    ...
    private void Get_DealersReportAlternate_Excel()
    {
        try
        {
            DateTime dtfrom = Convert.ToDateTime(txtFromDate.Text);
            ...
            if (ddlConsultant.SelectedValue == "0")
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    dt.Rows.Add(totalRow);
                    Utility.ExportToExcelDT(dt, "Alternate Report From " + ((Convert.ToDateTime(txtToDate.Text)).Year - 2) + " to " + (Convert.ToDateTime(txtToDate.Text)).Year);
                }
                else
                {
                    Utility.ShowMessage_Error(Page, "No Data found !");
