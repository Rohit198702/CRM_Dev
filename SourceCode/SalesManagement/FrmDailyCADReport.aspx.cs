...
public partial class SalesManagement_DailyCADReport : System.Web.UI.Page
{
    ...
    protected void gvDetail_RowEditing(object sender, GridViewEditEventArgs e)
    {
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                HfPNumber.Value = row["PNumber"].ToString();
        ...
        try
        {
            if (ValidationCheck())
            {
                ...
                if (btnSave.Text == "Save")
                {
                    ...
                    if (return_msg.Trim() == "ER02")
                    {
                        Utility.ShowMessage_Error(Page, "Proposal # not selected !");
                        HfPNumber.Value = ddlPNumberHeaderList.SelectedValue;
                        return;
                    }
    ...
    private void AfterSaveUpdateOperation(string return_msg, int saveOrUpdate)
    {
        ...
        {
            Utility.AddEditException(ex, return_msg);
        }
    }
    ...
    private void NatureOfTaskCheck()
    {
        try
        {
            if (ddlNature.SelectedValue == "1" && ddlPNumberHeaderList.SelectedIndex > 0)
            {
                ObjBOL.Operation = 19;
                ObjBOL.PNumber = HfPNumber.Value;
                ...
                {
                    Utility.ShowMessage_Error(Page, "New J already exists !");
                }
            }
        }
        catch (Exception ex)
