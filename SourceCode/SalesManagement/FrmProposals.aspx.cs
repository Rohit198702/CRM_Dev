...
public partial class SalesManagement_FrmProposals : System.Web.UI.Page
{
        ...
        try
        {
            if (!IsPostBack)
            {
                if (Utility.IsAuthorized())
                {
                    ...
                    {
                        string pnum = Request.QueryString["PNumber"].ToString();
                    ...
                    if (Request.QueryString["BDMActivity"] != null)
                    {
                        BDMActivitySpecLoad(Request.QueryString["BDMActivity"]);
                    }
    ...
    private void BDMActivitySpecLoad(string id)
    {
        try
        {
            btnAdd_Click();
            ...
            ObjBOL.Operation = 42;
            ObjBOL.StateID = Int32.Parse(id);
            ds = ObjBLL.GetFollowUpGrid(ObjBOL);
            //cls.Return_DS(ds, "EXEC aero_ManageBDMActivity 20, " + id);
            if (ds.Tables[0].Rows.Count == 1)
            {
                DataRow dr = ds.Tables[0].Rows[0];

                if (ddlIndustry.Items.FindByValue(dr["IndustryId"].ToString()) != null)
                {
                    ddlIndustry.SelectedValue = dr["IndustryId"].ToString();
                ...
                if (ddlStateAb.Items.FindByValue(dr["StateID"].ToString()) != null)
                {
                    ddlStateAb.SelectedValue = dr["StateID"].ToString();
                }
                ...
                txtEqPrice.Text = dr["EstimatedValue"].ToString();
                txtNetEqPrice.Text = dr["EstimatedValue"].ToString();
                txtTotalAmount.Text = dr["EstimatedValue"].ToString();
                hfSpecID.Value = dr["SpecID"].ToString();
            }
        }
        catch (Exception ex)
        {
    ...
    private void FillDetailsFromPnumber(string strPNumber)
    {
        try
        {
                ...
                Dictionary<string, Action<DataRow>> assignments = new Dictionary<string, Action<DataRow>>
            {
                ...
                { "SpecID", d => hfSpecID.Value = Convert.ToString(d["SpecID"]) },
                { "CurrencyID", d =>
                    {
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            if (ValidationCheck() == true)
            {
                ...
                int specID;
                if (int.TryParse(hfSpecID.Value, out specID))
                {
                    ObjBOL.SpecID = specID;
                }
                if (ddlState.SelectedIndex > 0)
                {
                ...
                if (txtDisPer.Text != "" && txtDisPer.Text != ".")
                {
                    ObjBOL.EqDiscount = Utility.ToDouble(txtDisPer.Text);
                }
                if (txtDisAmount.Text != "" && txtDisAmount.Text != ".")
                {
                    ObjBOL.EqDisAmount = Utility.ToDouble(txtDisAmount.Text);
                }
                if (txtNetEqPrice.Text != "" && txtNetEqPrice.Text != ".")
                {
                ...
                if (txtSpecAmount.Text != "" && txtSpecAmount.Text != ".")
                {
                    ...
                    else if (msg == "1")
                    {
                        ...
                        if (Utility.InventoryEmailSwitch())
                        {
                            if (hdfPM.Value != ddlProjectManager.SelectedValue)
                            {
                                ...
                                Utility.MaintainLogsSpecial("FrmProposals", "UpdatePM", text);
                            }
                        }
            ...
            {
                Utility.AddEditException(exs, txtProNO.Text);
            }
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex, txtProNO.Text);
        }
    }
    ...
    private void SendEmail()
    {
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                Message += "<p style = 'margin-top:5px'>Updated Time : </strong>" + DateTime.Now.ToString("dddd, dd MMMM yyyy HH:mm:ss") + "</p>";
    ...
    private void SendEmailSunil()
    {
        try
        {
            ...
            if (ddlProjectBid.SelectedValue == "4" || ddlProjectBid.SelectedValue == "7")
            {
                ...
                List<MailAddress> sendToList = new List<MailAddress>();
                sendToList.Add(sendtoSunil);
                Send_Email(Message, "Project status changed to " + ddlProjectBid.SelectedItem.Text + "", sendToList);
                Message = "";
            }
        }
        catch (Exception ex)
