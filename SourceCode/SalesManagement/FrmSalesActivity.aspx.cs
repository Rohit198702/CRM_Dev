    ...
    protected void gvFollowups_RowCommand(object sender, System.Web.UI.WebControls.GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName == "EditFollowup")
            {
                ...
                var dataKeyValue = gvFollowups.DataKeys[rowIndex].Values[0].ToString();
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    hfTimeStampFollowUps.Value = dr["TimeStampFollowUps"].ToString();
                }
        ...
        try
        {
            if (ValidationCheck_Followup())
            {
                ...
                else if (btnAddFollowup.Text == "Update")
                {
                    ...
                    if(hfTimeStampFollowUps.Value != "-1")
                    {
                        ObjBOL.TimeStampFollowUps = long.Parse(hfTimeStampFollowUps.Value);
                    }
                }
