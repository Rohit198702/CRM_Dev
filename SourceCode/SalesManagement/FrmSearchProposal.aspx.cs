...
public partial class SalesManagement_FrmSearchProposal : System.Web.UI.Page
{
    ...
    private void BindControls()
    {
        try
        {
            ...
            if(ddlDealerMember.Items.Count == 0)
            {
                ResetDealerMember();
            }
            if(ddlConsultantMember.Items.Count == 0)
            {
                ResetConsultantMember();
            }
        }
        catch (Exception ex)
        {
    ...
    private void Reset()
    {
        try
        {
            ...
            rdbOrderForProposal.SelectedValue = "0";
    ...
    private void GridBind()
    {
        try
        {
            if (ValidationCheck() == true)
            {
                ...
                if (txtPNumber.Text != "")
                {
                    Qstr += " AND tblPFiles.PNumber = '" + txtPNumber.Text.Trim().Replace("'", "") + "' ";
                }
                ...
                if (txtCity.Text != "")
                {
                    Qstr += " AND tblPFiles.City LIKE '%" + txtCity.Text.Replace("'", "") + "%' ";
                }
                ...
                {
                    Qstr += " AND tblPFiles.Country LIKE '%" + ddlCountry.SelectedItem.Text.Replace("'", "") + "%' ";
                }
                ...
                {
                    Qstr += " AND tblConsultantMembers.ConsultantMemberID='" + ddlConsultantMember.SelectedValue + "' ";
                }
                ...
                {
                    Qstr += " AND tblDealerMember.ContactID='" + ddlDealerMember.SelectedValue + "' ";
                }
                ...
                if (rdbOrderForProposal.SelectedValue == "1" || rdbOrderForProposal.SelectedValue == "2")
                {
                    Qstr += " AND tblPFiles.OrderBelongsTo= '" + rdbOrderForProposal.SelectedValue + "'";
                }
                if (!chkExactMatch.Checked)
                {
                    ...
                    foreach (ListItem item in ddlModelNewMultiSelectList.Items)
                    {
                        if (item.Selected && index == 0)
                        {
                           ...
                           (
                               SELECT 1
                               FROM tblSelectedModels sm
                               WHERE sm.PNumber = tblPFiles.PNumber
                               AND sm.ChildModelID IN (";
                    ...
                    foreach (ListItem item in ddlModelNewMultiSelectList.Items)
                    {
                        if (item.Selected)
                        {
                            ...
                            (
                                ...
                                FROM tblSelectedModels sm
                                WHERE sm.PNumber = tblPFiles.PNumber
                                AND sm.ChildModelID = " + item.Value + @"
                            )";
                        }
                    }
                }
    ...
    private void BindDealerMember(string DealerID)
    {
        try
        {
            DataSet ds = new DataSet();
            ObjBOL.Operation = 5;
            ObjBOL.dealerID = Convert.ToInt32(DealerID);
            ds = ObjBLL.GetProposalSearch(ObjBOL);
            if (ds.Tables[0].Rows.Count > 0)
            {
                Utility.BindDropDownListAll(ddlDealerMember, ds.Tables[0]);
            }
            else
            {
                ResetDealerMember();
    ...
    protected void ddldealer_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            if (ddldealer.SelectedIndex > 0)
            {
                ResetGrid();
                lblRecordsCount.Visible = false;
                BindDealerMember(ddldealer.SelectedValue);
            }
            else
            {
                ResetDealerMember();
    ...
    private void ResetDealerMember()
    {
        try
        {
            ddlDealerMember.DataSource = "";
            ddlDealerMember.DataBind();
            ddlDealerMember.Items.Insert(0, new ListItem("All", "0"));
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
    ...
    private void ResetConsultantMember()
    {
        try
        {
            ddlConsultantMember.DataSource = "";
            ddlConsultantMember.DataBind();
            ddlConsultantMember.Items.Insert(0, new ListItem("All", "0"));
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
    ...
    private void BindConsultantMember(string ConsultantID)
    {
        try
        {
            DataSet ds = new DataSet();
            ObjBOL.Operation = 6;
            ObjBOL.consultantID = Convert.ToInt32(ConsultantID);
            ds = ObjBLL.GetProposalSearch(ObjBOL);
            if (ds.Tables[0].Rows.Count > 0)
            {
                Utility.BindDropDownListAll(ddlConsultantMember, ds.Tables[0]);
            }
            else
            {
                ResetConsultantMember();
    ...
    protected void ddlConsultant_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            if (ddlConsultant.SelectedIndex > 0)
            {
                ResetGrid();
                lblRecordsCount.Visible = false;
                BindConsultantMember(ddlConsultant.SelectedValue);
            }
            else
            {
                ResetConsultantMember();
