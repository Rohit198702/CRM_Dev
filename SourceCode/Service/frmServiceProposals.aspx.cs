...
public partial class Service_frmServiceProposals : System.Web.UI.Page
{
        ...
        try
        {
            if (ValidationCheck() == true)
            {
                ...
                if (msg.Trim() != "")
                {
                    ...
                    //Bind_Grid();
                    ddlPNumber_SelectedIndexChanged();
                }
            }
        }
        catch (Exception ex)
    ...
    private void ddlPNumber_SelectedIndexChanged()
    {
        try
        {
            if (ddlPNumber.SelectedIndex > 0)
    ...
    protected void gvProposalDetails_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        {
            string msg = "";
            int ID = Convert.ToInt32(gvProposalDetails.DataKeys[e.RowIndex].Values[0]);
            ObjBOL.Operation = 10;
            ObjBOL.SerProposalDetailid = ID;
            GridViewRow row = gvProposalDetails.Rows[e.RowIndex];
            ...
            ObjBOL.Timestamp = long.Parse(gvProposalDetails.DataKeys[e.RowIndex].Values[1].ToString());
