...
public partial class TurboWash_FrmITWProjectsReport_V1 : System.Web.UI.Page
{
    ...
    private void BindControls()
    {
        try
        {
            ...
            commonClass.Return_DS(ds, "Exec Get_ITWProjects_V1_Report 2 ");
            if (ds.Tables[0].Rows.Count > 0)
            {
                Utility.BindDropDownListAll(ddlPOType, ds.Tables[0]);
            }

            if (ds.Tables[1].Rows.Count > 0)
            {
                Utility.BindDropDownListAll(ddlPONumber, ds.Tables[1]);
            }
        }
        catch (Exception ex)
        {
    ...
    private DataTable ReportData()
    {
        ...
        try
        {
            ...
            {
                query += ", " + ddlPONumber.SelectedValue;
            }
            else
            {
                query += ", '' ";
            ...
            query += ", " + ddlDateFilterOn.SelectedValue;
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ddlDateFilterOn.SelectedIndex = 0;
            ddlDateFilterOn_SelectedIndexChanged();
        }
        catch (Exception ex)
        {
            Utility.AddEditException(ex);
    ...
    protected void ddlDateFilterOn_SelectedIndexChanged(object sender, EventArgs e)
    {
        ddlDateFilterOn_SelectedIndexChanged();
    }

    protected void ddlDateFilterOn_SelectedIndexChanged()
    {
        try
        {
            if (ddlDateFilterOn.SelectedValue == "1")
            {
                lblFrom.InnerText = "PO Received Date From";
                lblTo.InnerText = "PO Received Date To";
            }
            else if (ddlDateFilterOn.SelectedValue == "2")
            {
                lblFrom.InnerText = "Ship Date From";
                lblTo.InnerText = "Ship Date To";
            }
        }
        catch (Exception ex)
        {
