...
public partial class TurboWash_FrmITWProjects_V1 : System.Web.UI.Page
{
    ...
    private bool ValidationCheck()
    {
        try
        {
            ...
            {
                Utility.ShowMessage_Error(Page, "Please Enter PO# !");
                txtPONumber.Focus();
                return false;
            }
            if (txtJobId.Text.Trim() == "" && txtPONumber.Text.Trim() == "")
            {
            ...
            if (txtShipDate.Text.Trim() == "" && ddlPOType.SelectedValue.Trim() == "1")
            {
    ...
    private void FetchInfo(DataSet ds)
    {
        try
        {
            ...
            Dictionary<string, Action<DataRow>> assignments = new Dictionary<string, Action<DataRow>>
            {
                ...
                { "ReleaseToNesting", d => {
                        if(d["POType"].ToString().Trim() == "1") {
                            chkReleaseToNesting.Enabled = true;
                        }
                        else
                        {
                            chkReleaseToNesting.Enabled = false;
                        }
                        chkReleaseToNesting.Checked = Convert.ToBoolean(d["ReleaseToNesting"]);
                    }
                },
    ...
    protected void btnAddShipmentDetail_Click(object sender, EventArgs e)
    {
        try
        {
            if (ValidationCheckShipment())
            {
                ...
                if (chkReleaseToNesting_Shipment.Checked)
                {
                    ObjBOL.ReleaseToNesting = true;
                }
    ...
    private void btnCancelShipmentDetail_Click()
    {
        try
        {
            ...
            chkReleaseToNesting_Shipment.Checked = false;
    ...
    protected void gvShipmentHistory_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            else if (e.CommandName == "ReleaseToNesting")
            {
                if (permissions.Trim() == "S")
                {
                    GridViewRow gvr = (GridViewRow)((LinkButton)e.CommandSource).NamingContainer;
                    ...
                    if (Utility.IsAuthorized())
                    {
                        ...
                        if (returnStatus.Trim() == "ER03")
                        {
                            Utility.ShowMessage_Error(Page, "Please release shipment to Fab. First !!");
                            return;
                        }

                        if (returnStatus.Trim() == "S")
                        {
                            Utility.MaintainLogsSpecial(formName, "Release-Nesting", selectedIds);
                            Utility.ShowMessage_Success(Page, "Shipments Released Successfully !!");
                            GetShipmentHistory();
                        }
                    }
    ...
    protected void gvShipmentHistory_RowEditing(object sender, GridViewEditEventArgs e)
    {
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                txtRequestedReceiveDate.Text = row["RequestedReceiveDate"].ToString();
                chkReleaseToNesting_Shipment.Checked = Convert.ToBoolean(row["ReleaseToNesting"]);
